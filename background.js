...
importScripts(
  ...
  'utils/managed-alias-utils.js',
  'utils/mail2925-utils.js',
  ...
  'background/sensitive-settings.js',
  ...
  'background/steps/cookie-cleanup.js',
  ...
  'data/names.js',
  'utils/hotmail-utils.js',
  'utils/microsoft-email.js',
  'utils/luckmail-utils.js',
  'utils/cloudflare-temp-email-utils.js',
  'utils/cloudmail-utils.js',
  'background/cloudmail-provider.js',
  'utils/yyds-mail-utils.js',
  'background/yyds-mail-provider.js',
  'utils/icloud-utils.js',
  'utils/mail-provider-utils.js',
  'content/activation-utils.js'
);

const sensitiveSettingsHelpers = self.MultiPageSensitiveSettings || {};
const omitLocalOnlySettings = typeof sensitiveSettingsHelpers.omitLocalOnlySettings === 'function'
  ? sensitiveSettingsHelpers.omitLocalOnlySettings
  : ((state = {}) => ({ ...(state || {}) }));
const buildStateStorageView = typeof sensitiveSettingsHelpers.buildStateStorageView === 'function'
  ? sensitiveSettingsHelpers.buildStateStorageView
  : ((options = {}) => ({
    ...(options.defaultState || {}),
    ...(options.persistedSettings || {}),
    ...(options.persistedAliasState || {}),
    ...(options.sessionState || {}),
    accountRunHistory: Array.isArray(options.accountRunHistory) ? options.accountRunHistory : [],
  }));
...
const NORMAL_STEP_IDS = NORMAL_STEP_DEFINITIONS
  ...
  .filter(Number.isFinite)
  .sort((left, right) => left - right);
const LAST_STEP_ID = NORMAL_STEP_IDS[NORMAL_STEP_IDS.length - 1] || 10;
...
function isStateValueEqual(left, right) {
...
function pickChangedStatePatch(state = {}, patch = {}) {
  const changed = {};
  for (const [key, value] of Object.entries(patch || {})) {
    if (!isStateValueEqual(state?.[key], value)) {
      changed[key] = value;
    }
  }
  return changed;
...
function statePatchHasChanges(state = {}, patch = {}) {
  return Object.keys(pickChangedStatePatch(state, patch)).length > 0;
}
...
const AUTO_RUN_EXECUTION_MODE_FULL = 'full';
const AUTO_RUN_EXECUTION_MODE_REGISTER_GPT = 'register_gpt';
const AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX = 'authorize_codex';
const AUTO_RUN_DELAY_MIN_MINUTES = 0;
...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  executionMode: AUTO_RUN_EXECUTION_MODE_FULL, // 执行流程模式：full / register_gpt / authorize_codex。
  importedAccounts: [], // 从侧边栏“2.导入账号”读取的账号邮箱数组。
  startStep: 1, // 自动运行的起始步骤编号。
  endStep: 11, // 自动运行的结束步骤编号。
  authorizationEmail: '', // 授权阶段（7-11）本轮使用的邮箱。
  ...
  gptmailApiKey: '',
  gptmailBaseUrl: DEFAULT_GPTMAIL_BASE_URL,
  gptmailDomain: '',
  cloudflareDomain: '',
  cloudflareDomains: [],
  ...
  // Cloudflare Temp Email 自生成邮箱时固定 /admin/mails 查询的完整收件箱地址。
  cloudflareTempEmailFixedMailbox: '',
...
// 单位毫秒：步骤 6 固定等待 2 秒。
const STEP6_REGISTRATION_SUCCESS_WAIT_MS = 2000;
...
async function ensureResolvedSignupMethodForRun(options = {}) {
  ...
  if (!force && existing === SIGNUP_METHOD_EMAIL) {
...
function normalizeAutoRunTimerPlan(plan) {
  ...
  const normalizeExecutionMode = typeof normalizeAutoRunExecutionMode === 'function'
    ? normalizeAutoRunExecutionMode
    : ((value = '') => {
      const normalized = String(value || '').trim().toLowerCase();
      if (normalized === 'register_gpt') return 'register_gpt';
      if (normalized === 'authorize_codex') return 'authorize_codex';
      return 'full';
    });
  const normalizeImportedAccountsSafe = typeof normalizeImportedAccounts === 'function'
    ? normalizeImportedAccounts
    : ((value = []) => (Array.isArray(value)
      ? value.map((item) => String(item || '').trim()).filter(Boolean)
      : []));
  const normalizeStepRange = typeof normalizeAutoRunStepRange === 'function'
    ? normalizeAutoRunStepRange
    : ((mode = 'full', startStep, endStep) => {
      const normalizedMode = normalizeExecutionMode(mode);
      const defaults = normalizedMode === 'register_gpt'
        ? { startStep: 1, endStep: 6 }
        : (normalizedMode === 'authorize_codex'
          ? { startStep: 7, endStep: 11 }
          : { startStep: 1, endStep: 11 });
      let normalizedStartStep = Math.max(1, Math.floor(Number(startStep) || defaults.startStep));
      let normalizedEndStep = Math.max(1, Math.floor(Number(endStep) || defaults.endStep));
      if (normalizedStartStep > normalizedEndStep) {
        normalizedStartStep = defaults.startStep;
        normalizedEndStep = defaults.endStep;
      }
      return {
  ...
  const executionMode = normalizeExecutionMode(plan.executionMode);
  const importedAccounts = normalizeImportedAccountsSafe(plan.importedAccounts);
  const executionStepRange = normalizeStepRange(executionMode, plan.startStep, plan.endStep);
  const authorizationEmail = String(plan.authorizationEmail || '').trim();
...
function normalizeAutoRunTimerPlanFromState(state = {}) {
  ...
  return normalizeAutoRunTimerPlan({
    ...
    executionMode: state.scheduledAutoRunPlan?.executionMode ?? state.executionMode,
    importedAccounts: state.scheduledAutoRunPlan?.importedAccounts ?? state.importedAccounts,
    startStep: state.scheduledAutoRunPlan?.startStep ?? state.startStep,
    endStep: state.scheduledAutoRunPlan?.endStep ?? state.endStep,
    authorizationEmail: state.scheduledAutoRunPlan?.authorizationEmail ?? state.authorizationEmail,
  });
}
...
async function markCurrentRegistrationAccountUsed(state = {}, options = {}) {
  ...
  const isCurrentGptmailProvider = typeof isGptmailProvider === 'function'
    ? isGptmailProvider(latestState)
    : String(latestState?.mailProvider || '').trim().toLowerCase() === 'gptmail';
  if (isCurrentGptmailProvider) {
    const currentAddress = normalizeGptmailEmailAddress(latestState.currentGptmailAddress || latestState.email);
    if (currentAddress) {
      await clearGptmailRuntimeState({ clearEmail: true });
      await addLog(`${reasonPrefix}：GPTMail 邮箱 ${currentAddress} 已完成并清理。`, options.level || 'warn');
      updated = true;
    }
  }
...
function buildGptmailSessionSettingsPayload(input = {}) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    return {};
  }

  const payload = {};
  if (input.gptmailApiKey !== undefined) {
    payload.gptmailApiKey = String(input.gptmailApiKey || '');
  }
  if (input.gptmailBaseUrl !== undefined) {
    payload.gptmailBaseUrl = normalizeGptmailBaseUrl(input.gptmailBaseUrl);
  }
  if (input.gptmailDomain !== undefined) {
    payload.gptmailDomain = normalizeGptmailDomain(input.gptmailDomain);
  }
  if (input.currentGptmailAddress !== undefined) {
    payload.currentGptmailAddress = normalizeGptmailEmailAddress(input.currentGptmailAddress) || null;
  }
  if (input.currentGptmailMailCursor !== undefined) {
    payload.currentGptmailMailCursor = normalizeGptmailMailCursor(input.currentGptmailMailCursor);
  }
  return payload;
}
...
function getCloudflareTempEmailConfig(state = {}) {
  return {
    ...
    fixedMailbox: normalizeCloudflareTempEmailReceiveMailbox(state.cloudflareTempEmailFixedMailbox),
...
function resolveCloudflareTempEmailPollTargetEmail(state = {}, pollPayload = {}, config = getCloudflareTempEmailConfig(state)) {
  ...
  const fixedMailbox = normalizeCloudflareTempEmailReceiveMailbox(config.fixedMailbox);
  if (mailProvider === 'cloudflare-temp-email' && emailGenerator === 'cloudflare-temp-email' && fixedMailbox) {
    return fixedMailbox;
  }
...
function normalizeAutoRunExecutionMode(value = '') {
  const normalized = String(value || '').trim().toLowerCase();
  if (normalized === AUTO_RUN_EXECUTION_MODE_REGISTER_GPT) {
    return AUTO_RUN_EXECUTION_MODE_REGISTER_GPT;
  }
  if (normalized === AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX) {
    return AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX;
  }
  return AUTO_RUN_EXECUTION_MODE_FULL;
}
...
function normalizeAutoRunStepBoundary(value, fallback = 0) {
...
function getDefaultAutoRunStepRangeByMode(mode = AUTO_RUN_EXECUTION_MODE_FULL) {
  const normalizedMode = normalizeAutoRunExecutionMode(mode);
  if (normalizedMode === AUTO_RUN_EXECUTION_MODE_REGISTER_GPT) {
    return { startStep: 1, endStep: 6 };
  }
  if (normalizedMode === AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX) {
    return { startStep: 7, endStep: 11 };
  }
  return { startStep: 1, endStep: 11 };
}

function normalizeAutoRunStepRange(mode = AUTO_RUN_EXECUTION_MODE_FULL, startStep, endStep) {
  const defaultRange = getDefaultAutoRunStepRangeByMode(mode);
  let normalizedStartStep = normalizeAutoRunStepBoundary(startStep, defaultRange.startStep);
  let normalizedEndStep = normalizeAutoRunStepBoundary(endStep, defaultRange.endStep);
  if (normalizedStartStep > normalizedEndStep) {
...
function normalizePersistentSettingValue(key, value) {
  switch (key) {
    ...
    case 'executionMode':
      return normalizeAutoRunExecutionMode(value);
    case 'importedAccounts':
      return normalizeImportedAccounts(value);
    case 'autoRunSkipFailures':
    case 'oauthFlowTimeoutEnabled':
    case 'autoRunDelayEnabled':
      return Boolean(value);
    case 'startStep':
      return normalizeAutoRunStepBoundary(value, PERSISTED_SETTING_DEFAULTS.startStep);
    case 'endStep':
      return normalizeAutoRunStepBoundary(value, PERSISTED_SETTING_DEFAULTS.endStep);
    case 'authorizationEmail':
      return String(value || '').trim();
    case 'operationDelayEnabled':
      return typeof value === 'boolean' ? value : true;
    case 'step6CookieCleanupEnabled':
    case 'isAccountWriteEnabled':
      return Boolean(value);
    case 'accountWriteFlowType': {
      const normalized = String(value || '').trim().toLowerCase();
    ...
    case 'accountWriteFileName':
      return String(value || '').trim();
    case 'accountWriteSelectedAt':
    ...
    case 'gptmailApiKey':
      return String(value || '');
    case 'gptmailBaseUrl':
      return normalizeGptmailBaseUrl(value);
    case 'gptmailDomain':
      return normalizeGptmailDomain(value);
    case 'yydsMailApiKey':
    ...
    case 'cloudflareTempEmailFixedMailbox':
...
function buildPersistentSettingsPayload(input = {}, options = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(payload, 'signupMethod')
...
let sessionStateCacheLoadPromise = null;
let persistedSettingsCache = null;
let persistedSettingsCacheLoadPromise = null;
let persistedAliasStateCache = null;
let persistedAliasStateCacheLoadPromise = null;

function invalidateSessionStateCache() {
  sessionStateCache = null;
}

function invalidatePersistedSettingsCache() {
  persistedSettingsCache = null;
}

function invalidatePersistedAliasStateCache() {
  persistedAliasStateCache = null;
}

function mergeSessionStateCache(updates = {}) {
  const current = sessionStateCache && typeof sessionStateCache === 'object' && !Array.isArray(sessionStateCache)
    ? sessionStateCache
    : {};
  sessionStateCache = {
    ...current,
...
async function getSessionStateSnapshot(options = {}) {
  ...
  if (!sessionStateCacheLoadPromise) {
    sessionStateCacheLoadPromise = chrome.storage.session.get(null)
      .then((stored) => {
        sessionStateCache = stored && typeof stored === 'object' && !Array.isArray(stored)
          ? { ...stored }
          : {};
        return sessionStateCache;
      })
      .finally(() => {
        sessionStateCacheLoadPromise = null;
      });
  }
  const snapshot = await sessionStateCacheLoadPromise;
  return { ...(snapshot || {}) };
}

async function getPersistedSettings() {
  if (persistedSettingsCache) {
    return { ...persistedSettingsCache };
  }
  if (!persistedSettingsCacheLoadPromise) {
    persistedSettingsCacheLoadPromise = chrome.storage.local.get([
      ...PERSISTED_SETTING_KEYS,
      ...LEGACY_AUTO_STEP_DELAY_KEYS,
      ...LEGACY_VERIFICATION_RESEND_COUNT_KEYS,
    ])
      .then((stored) => {
        persistedSettingsCache = buildPersistentSettingsPayload(stored, { fillDefaults: true });
        return persistedSettingsCache;
      })
      .finally(() => {
        persistedSettingsCacheLoadPromise = null;
      });
  }
  const settings = await persistedSettingsCacheLoadPromise;
  return { ...(settings || {}) };
}

async function getPersistedAliasState() {
  if (persistedAliasStateCache) {
    return {
      ...persistedAliasStateCache,
    };
  }
  if (persistedAliasStateCacheLoadPromise) {
    const cached = await persistedAliasStateCacheLoadPromise;
    return { ...(cached || {}) };
  }
  try {
    persistedAliasStateCacheLoadPromise = chrome.storage.local.get(PERSISTENT_ALIAS_STATE_KEYS)
      .then((stored) => {
        ...
        return persistedAliasStateCache;
      })
      .finally(() => {
        persistedAliasStateCacheLoadPromise = null;
      });
    const aliasState = await persistedAliasStateCacheLoadPromise;
    return { ...(aliasState || {}) };
  } catch (err) {
    ...
    persistedAliasStateCache = fallback;
    return { ...fallback };
  }
}
...
async function getState() {
  ...
  return buildStateViewWithRuntimeState(buildStateStorageView({
...
function setupStorageStateCacheSync() {
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (areaName === 'session') {
      ...
      sessionStateCache = current;
      return;
    }
    if (areaName === 'local') {
      invalidatePersistedSettingsCache();
      invalidatePersistedAliasStateCache();
    }
  });
}

async function setState(updates) {
  const debugUpdates = updates && typeof updates === 'object' && !Array.isArray(updates)
    ? {
      ...updates,
      // logs 可能非常大，调试输出只打印条数，避免高频 JSON.stringify 造成额外开销。
      ...(Object.prototype.hasOwnProperty.call(updates, 'logs')
        ? { logs: `[${Array.isArray(updates.logs) ? updates.logs.length : 0} entries]` }
        : {}),
      ...(Object.prototype.hasOwnProperty.call(updates, 'duckApiAuthorization')
        ? { duckApiAuthorization: '[REDACTED]' }
        : {}),
    }
    : updates;
  console.log(LOG_PREFIX, 'storage.set:', JSON.stringify(debugUpdates).slice(0, 200));
  if (Object.keys(updates || {}).length > 0) {
    const currentSessionState = await getSessionStateSnapshot();
    const projectedSessionState = {
      ...DEFAULT_STATE,
      ...currentSessionState,
    };
    const sessionPatch = omitLocalOnlySettings(buildStatePatchWithRuntimeState(projectedSessionState, updates));
    const changedSessionUpdates = pickChangedStatePatch(projectedSessionState, sessionPatch);
    if (Object.keys(changedSessionUpdates).length <= 0) {
      return;
    }
    await chrome.storage.session.set(changedSessionUpdates);
    mergeSessionStateCache(changedSessionUpdates);
    const persistentAliasUpdates = {};
    if (Object.prototype.hasOwnProperty.call(changedSessionUpdates, 'manualAliasUsage')) {
      persistentAliasUpdates.manualAliasUsage = normalizeBooleanMap(changedSessionUpdates.manualAliasUsage);
    }
    if (Object.prototype.hasOwnProperty.call(changedSessionUpdates, 'preservedAliases')) {
      persistentAliasUpdates.preservedAliases = normalizeBooleanMap(changedSessionUpdates.preservedAliases);
    }
    if (Object.prototype.hasOwnProperty.call(changedSessionUpdates, 'icloudAliasCache')) {
      persistentAliasUpdates.icloudAliasCache = normalizeIcloudAliasCacheList(changedSessionUpdates.icloudAliasCache);
    }
    if (Object.prototype.hasOwnProperty.call(changedSessionUpdates, 'icloudAliasCacheAt')) {
      persistentAliasUpdates.icloudAliasCacheAt = Math.max(0, Number(changedSessionUpdates.icloudAliasCacheAt) || 0);
    }
...
// 日志持久化轻量路径：仅更新 session logs，避免通用 setState 的全量状态拼装。
async function setLogsState(logs = []) {
  const normalizedLogs = Array.isArray(logs) ? logs : [];
  await chrome.storage.session.set({ logs: normalizedLogs });
  mergeSessionStateCache({ logs: normalizedLogs });
}
...
async function setPersistentSettings(updates) {
  ...
  if (Object.keys(persistedUpdates).length > 0) {
    ...
    const changedPersistedUpdates = pickChangedStatePatch(currentSettings, persistedUpdates);
    if (Object.keys(changedPersistedUpdates).length <= 0) {
      return;
    }
    await chrome.storage.local.set(changedPersistedUpdates);
    persistedSettingsCache = {
      ...currentSettings,
      ...changedPersistedUpdates,
    };
  }
}
...
async function importSettingsBundle(configBundle) {
  ...
  ['duckApiAuthorization'].forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(sessionUpdates, key)) {
      delete sessionUpdates[key];
    }
  });

  await setState(sessionUpdates);
  const importBroadcastPayload = {
    ...importedSettings,
    currentHotmailAccountId: null,
  ...
  ['duckApiAuthorization'].forEach((key) => {
    if (Object.prototype.hasOwnProperty.call(importBroadcastPayload, key)) {
      delete importBroadcastPayload[key];
    }
  });
  broadcastDataUpdate(importBroadcastPayload);
...
function normalizeAccountRecordIdentityType(value = '') {
  return String(value || '').trim().toLowerCase() === 'phone' ? 'phone' : 'email';
}

function normalizeAccountRecordIdentityValue(value = '', type = 'email') {
  const normalizedType = normalizeAccountRecordIdentityType(type);
  const normalizedValue = String(value || '').trim();
  if (!normalizedValue) {
    return '';
  }
  return normalizedType === 'phone'
    ? normalizedValue
    : normalizedValue.toLowerCase();
}

function resolveAccountRecordIdentityFromState(state = {}) {
  const accountIdentifierType = normalizeAccountRecordIdentityType(state?.accountIdentifierType);
  const fallbackIdentifier = String(state?.email || '').trim();
  const rawAccountIdentifier = String(state?.accountIdentifier || fallbackIdentifier || '').trim();
  const accountIdentifier = normalizeAccountRecordIdentityValue(rawAccountIdentifier, accountIdentifierType);
  return {
...
function hasRunningAccountRecordForIdentity(state = {}, identity = {}) {
  const targetType = normalizeAccountRecordIdentityType(identity?.accountIdentifierType);
  const targetIdentifier = normalizeAccountRecordIdentityValue(identity?.accountIdentifier, targetType);
  if (!targetIdentifier) {
    return false;
  }
  const history = Array.isArray(state?.accountRunHistory) ? state.accountRunHistory : [];
  return history.some((record) => {
    const recordType = normalizeAccountRecordIdentityType(
      record?.accountIdentifierType
      || (String(record?.phoneNumber || '').trim() && !String(record?.email || '').trim() ? 'phone' : 'email')
    );
    const recordIdentifier = normalizeAccountRecordIdentityValue(
      record?.accountIdentifier || (recordType === 'phone' ? record?.phoneNumber : record?.email),
      recordType
    );
    const finalStatus = String(record?.finalStatus || record?.status || '').trim().toLowerCase();
...
async function finalizePreviousRunningAccountRecordOnSwitch(stateBeforeSwitch = {}, nextIdentity = {}, reason = '') {
  if (!shouldMarkAccountRunRecordRunning(stateBeforeSwitch)) {
    return;
  }

  const previousIdentity = resolveAccountRecordIdentityFromState(stateBeforeSwitch);
  const nextType = normalizeAccountRecordIdentityType(nextIdentity?.accountIdentifierType);
  const nextIdentifier = normalizeAccountRecordIdentityValue(nextIdentity?.accountIdentifier, nextType);

  if (!previousIdentity.accountIdentifier || !nextIdentifier) {
    return;
  }
  if (previousIdentity.accountIdentifierType === nextType && previousIdentity.accountIdentifier === nextIdentifier) {
    return;
  }
  if (!hasRunningAccountRecordForIdentity(stateBeforeSwitch, previousIdentity)) {
    return;
  }

  const resolvedReason = String(reason || '').trim() || '自动流程已切换账号，上一账号结束当前运行。';
  await appendManualAccountRunRecordIfNeeded('node:submit-signup-email:stopped', stateBeforeSwitch, resolvedReason);
}

async function setEmailState(email, options = {}) {
  const stateBeforeSwitch = await getState();
  const preserveAccountIdentity = Boolean(options?.preserveAccountIdentity);
  const previousIdentity = resolveAccountRecordIdentityFromState(stateBeforeSwitch);
  const nextIdentity = preserveAccountIdentity && previousIdentity.accountIdentifier
    ? previousIdentity
    : {
      accountIdentifierType: 'email',
      accountIdentifier: String(email || '').trim(),
    };
  await finalizePreviousRunningAccountRecordOnSwitch(stateBeforeSwitch, nextIdentity);
...
function normalizeGptmailEmailAddress(value = '') {
...
function normalizeGptmailMailCursor(value = {}) {
  const ids = Array.isArray(value?.seenIds) ? value.seenIds : [];
  return {
    seenIds: Array.from(new Set(ids.map((id) => String(id || '').trim()).filter(Boolean))),
  };
}

function normalizeGptmailUsage(value = null) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return null;
  }
  const usage = {};
  for (const key of ['daily_limit', 'used_today', 'remaining_today', 'total_limit', 'total_usage', 'remaining_total']) {
...
async function setGptmailAddressState(email) {
  const normalizedEmail = normalizeGptmailEmailAddress(email);
  await setState({ currentGptmailAddress: normalizedEmail || null });
  broadcastDataUpdate({ currentGptmailAddress: normalizedEmail || null });
  return normalizedEmail;
}

async function setGptmailMailCursorState(cursor) {
  const normalizedCursor = normalizeGptmailMailCursor(cursor);
  await setState({ currentGptmailMailCursor: normalizedCursor });
  return normalizedCursor;
}

async function setGptmailUsageState(usage) {
  const normalizedUsage = normalizeGptmailUsage(usage);
  await setState({ gptmailUsage: normalizedUsage });
  broadcastDataUpdate({ gptmailUsage: normalizedUsage });
  return normalizedUsage;
}

async function clearGptmailRuntimeState(options = {}) {
...
async function resetState() {
  ...
  if (typeof invalidateSessionStateCache === 'function') {
    invalidateSessionStateCache();
  }
  const normalizeGptmailBaseUrlForReset = typeof normalizeGptmailBaseUrl === 'function'
    ? normalizeGptmailBaseUrl
    : ((value) => String(value || '').trim() || (
      typeof DEFAULT_GPTMAIL_BASE_URL !== 'undefined'
        ? DEFAULT_GPTMAIL_BASE_URL
        : 'https://mail.chatgpt.org.uk'
    ));
  const normalizeGptmailDomainForReset = typeof normalizeGptmailDomain === 'function'
    ? normalizeGptmailDomain
    : ((value) => {
      const normalized = String(value || '').trim().replace(/^@+/, '').toLowerCase();
      return /^[a-z0-9.-]+\.[a-z]{2,}$/.test(normalized) ? normalized : '';
    });
  const omitLocalOnlySettingsForReset = typeof omitLocalOnlySettings === 'function'
    ? omitLocalOnlySettings
    : ((state = {}) => {
        const next = { ...(state || {}) };
        delete next.duckApiAuthorization;
        return next;
      });
  const resetPayload = omitLocalOnlySettingsForReset(buildStatePatchWithRuntimeState({}, {
    ...DEFAULT_STATE,
    ...persistedSettings,
    ...persistedAliasState,
    ...
    gptmailApiKey: Object.prototype.hasOwnProperty.call(prev, 'gptmailApiKey')
      ? String(prev.gptmailApiKey || '')
      : String(persistedSettings.gptmailApiKey || ''),
    gptmailBaseUrl: normalizeGptmailBaseUrlForReset(
      Object.prototype.hasOwnProperty.call(prev, 'gptmailBaseUrl')
        ? prev.gptmailBaseUrl
        : persistedSettings.gptmailBaseUrl
    ),
    gptmailDomain: normalizeGptmailDomainForReset(
      Object.prototype.hasOwnProperty.call(prev, 'gptmailDomain')
        ? prev.gptmailDomain
        : persistedSettings.gptmailDomain
    ),
    currentLuckmailPurchase: null,
    currentLuckmailMailCursor: null,
    currentGptmailAddress: null,
  ...
  if (typeof mergeSessionStateCache === 'function') {
    mergeSessionStateCache(resetPayload);
  }
}
...
function isGptmailProvider(stateOrProvider) {
  const provider = typeof stateOrProvider === 'string'
    ? stateOrProvider
    : stateOrProvider?.mailProvider;
  return provider === GPTMAIL_PROVIDER;
}
...
function getGptmailSessionConfig(state = {}) {
  return {
    apiKey: String(state.gptmailApiKey || ''),
    baseUrl: normalizeGptmailBaseUrl(state.gptmailBaseUrl),
    domain: normalizeGptmailDomain(state.gptmailDomain),
  };
}

async function requestGptmail(path, { method = 'GET', baseUrl, apiKey, params, jsonData, timeout = 15000 } = {}) {
  const requestUrl = new URL(path, `${normalizeGptmailBaseUrl(baseUrl)}/`);
  if (params && typeof params === 'object') {
    for (const [key, value] of Object.entries(params)) {
      if (value === undefined || value === null || value === '') continue;
      requestUrl.searchParams.set(key, String(value));
  ...
  if (jsonData !== undefined && normalizedMethod !== 'GET') {
    fetchOptions.body = JSON.stringify(jsonData);
  ...
  try {
    ...
    if (err?.name === 'AbortError') {
      throw new Error(`GPTMail 请求超时：${path}`);
    }
    throw new Error(`GPTMail 请求失败：${err.message}`);
  } finally {
    clearTimeout(timeoutId);
  }
  ...
  try {
    ...
    throw new Error(`GPTMail 返回了无法解析的响应：${path}`);
  }

  if (!response.ok) {
    const errorText = String(payload?.error || payload?.message || response.statusText || 'HTTP error').trim();
    throw new Error(`GPTMail 请求失败：${errorText}`);
  }
  if (payload && payload.success === false) {
    const errorText = String(payload.error || payload.message || 'unknown error').trim();
    throw new Error(`GPTMail 接口返回失败：${errorText}`);
  }
  if (payload?.usage !== undefined && typeof setGptmailUsageState === 'function') {
    await setGptmailUsageState(payload.usage);
  }
  return payload && Object.prototype.hasOwnProperty.call(payload, 'data') ? payload.data : payload;
}

function createGptmailClient(state = {}) {
  const config = getGptmailSessionConfig(state);
  const request = (path, options = {}) => requestGptmail(path, {
  ...
  return {
    async generateEmail(options = {}) {
      const domain = normalizeGptmailDomain(options.domain);
      const data = domain
        ? await request('/api/generate-email', {
          method: 'POST',
          jsonData: { domain },
        })
        : await request('/api/generate-email');
      const email = normalizeGptmailEmailAddress(data?.email);
      if (!email) {
        throw new Error('GPTMail 返回了空邮箱地址。');
      }
      return email;
    },
    async listEmails(email) {
      const data = await request('/api/emails', {
        params: { email: normalizeGptmailEmailAddress(email) || String(email || '').trim() },
        timeout: 10000,
      });
    ...
    async getEmailDetail(messageId) {
      ...
      if (!id) return {};
      const data = await request(`/api/email/${encodeURIComponent(id)}`, { timeout: 10000 });
...
async function ensureGptmailAddressForFlow() {
  const state = await getState();
  const config = getGptmailSessionConfig(state);
  const existingEmail = normalizeGptmailEmailAddress(state.currentGptmailAddress);
  const existingDomain = String(existingEmail.split('@')[1] || '').toLowerCase();
  if (existingEmail && (!config.domain || existingDomain === config.domain)) {
    await setGptmailAddressState(existingEmail);
    if (!state.currentGptmailMailCursor) {
      await setGptmailMailCursorState({ seenIds: [] });
    }
    if (normalizeGptmailEmailAddress(state.email) !== existingEmail) {
      await setEmailState(existingEmail);
    }
  ...
  const email = await createGptmailClient(state).generateEmail(
    config.domain ? { domain: config.domain } : {}
  );
  const normalizedEmail = normalizeGptmailEmailAddress(email);
  if (!normalizedEmail) {
    throw new Error('GPTMail 未生成有效邮箱地址。');
  }

  await setGptmailAddressState(normalizedEmail);
  await setGptmailMailCursorState({ seenIds: [] });
  await setEmailState(normalizedEmail);
  await addLog(`GPTMail：已生成邮箱 ${normalizedEmail}`, 'ok');
  return normalizedEmail;
}
...
function stripGptmailHtmlForCode(html = '') {
  const rawHtml = String(html || '');
  const bodyMatch = rawHtml.match(/<body\b[^>]*>([\s\S]*?)<\/body>/i);
  const scopedHtml = bodyMatch ? bodyMatch[1] : rawHtml;
  return scopedHtml
    .replace(/<!--[\s\S]*?-->/g, ' ')
...
function extractGptmailVerificationCodeFromHtml(html = '') {
  const text = stripGptmailHtmlForCode(html);
  const contextualMatch = text.match(/(?:输入此临时验证码以继续|临时验证码|验证码|verification\s+code|temporary\s+code|code)[^\d]{0,120}(\d{6})(?!\d)/i);
  if (contextualMatch) {
    return contextualMatch[1];
  }
  return extractGptmailVerificationCode(text);
}

function isGptmailApiKeyUsageLimitMessage(message = '') {
  return /(?:GPTMail[\s\S]*)?API key usage limit reached/i.test(String(message || ''));
}

async function pollGptmailVerificationCode(step, state, pollPayload = {}) {
  const email = normalizeGptmailEmailAddress(
    pollPayload.targetEmail
    || state.currentGptmailAddress
    || state.email
  );
  if (!email) {
    throw new Error('GPTMail 当前没有可用邮箱，请先执行步骤 3 生成邮箱。');
  }

  const client = createGptmailClient(state);
  const maxAttempts = Math.max(1, Number(pollPayload.maxAttempts) || 3);
  const intervalMs = Math.max(DEFAULT_GPTMAIL_POLL_INTERVAL_MS, Number(pollPayload.intervalMs) || DEFAULT_GPTMAIL_POLL_INTERVAL_MS);
  const excludedCodes = new Set((pollPayload.excludeCodes || []).filter(Boolean).map(String));
  const cursor = normalizeGptmailMailCursor((await getState()).currentGptmailMailCursor);
  const seenIds = new Set(cursor.seenIds);
  let lastError = null;

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    throwIfStopped();
    await addLog(`步骤 ${step}：正在通过 GPTMail 轮询邮箱 ${email} 的验证码（${attempt}/${maxAttempts}）...`, 'info');
    try {
      const messages = await client.listEmails(email);
      for (const message of messages) {
        ...
        let detail = {};
        try {
          detail = await client.getEmailDetail(messageId);
        } catch (err) {
          if (isStopError(err) || isGptmailApiKeyUsageLimitMessage(err?.message)) {
            throw err;
          }
          await addLog(`步骤 ${step}：GPTMail 邮件详情读取失败：${err.message}`, 'warn');
        }
        const code = extractGptmailVerificationCodeFromHtml(detail?.html_content || '');
        if (!code) continue;
        if (excludedCodes.has(code)) {
          lastError = new Error(`步骤 ${step}：GPTMail 返回的验证码 ${code} 已试过，继续等待新邮件。`);
          continue;
        }
        await setGptmailMailCursorState({ seenIds: Array.from(seenIds) });
        return {
          ok: true,
          code,
          emailTimestamp: Date.parse(message?.received_at || message?.created_at || detail?.received_at || detail?.created_at || '') || Date.now(),
          mailId: messageId,
        };
      }
      await setGptmailMailCursorState({ seenIds: Array.from(seenIds) });
      lastError = new Error(`步骤 ${step}：暂未在 GPTMail 邮箱中找到新的匹配验证码。`);
    } catch (err) {
      if (isStopError(err)) {
        throw err;
      }
      lastError = err;
      await addLog(`步骤 ${step}：GPTMail 轮询失败：${err.message}`, 'warn');
      if (isGptmailApiKeyUsageLimitMessage(err?.message)) {
        throw err;
      }
    }
  ...
  throw lastError || new Error(`步骤 ${step}：未在 GPTMail 邮箱中获取到新的验证码。`);
}
  ...
  return withIcloudLoginHelp('获取 iCloud 隐私邮箱', async () => {
    ...
    try {
      ...
      const reserveData = {
        ...
        note: 'Generated through Codex Free 注册机',
      };
...
// 步骤 5 完成态放行页（与 master 对齐并补充 passkey enroll）。
function isStep5CompletionChatgptUrl(rawUrl) {
  // rawUrl：当前页面 URL（用于判断 Step5 提交后的放行跳转页面）。
  const parsed = parseUrlSafely(rawUrl);
  if (!parsed) return false;
  ...
  if (['chatgpt.com', 'www.chatgpt.com'].includes(hostname)) {
    return !/^\/(?:auth\/|create-account\/|email-verification|log-in|add-phone)(?:[/?#]|$)/i.test(pathname);
  }
  if (isSignupPageHost(hostname)) {
    return /\/create-account-enroll-passkey(?:[/?#]|$)/i.test(pathname);
  }
  return false;
}
...
function isGptmailApiKeyUsageLimitError(error) {
  return isGptmailApiKeyUsageLimitMessage(getErrorMessage(error));
}
...
function isAddPhoneAuthFailure(error) {
  ...
  return /ADD_PHONE_REQUIRED::|https:\/\/auth\.openai\.com\/add-phone(?:[/?#]|$)|\badd-phone\b|\u6dfb\u52a0\u624b\u673a\u53f7|\u624b\u673a\u53f7\u7801|\u8fdb\u5165\u624b\u673a\u53f7\u9875\u9762|\u624b\u673a\u53f7\u9875|\u624b\u673a\u53f7\u9875\u9762|phone\s+number|telephone/i.test(message);
}
...
function isPostLoginPhoneVerificationAddPhoneWarning(error, nodeId = '') {
  const normalizedNodeId = String(nodeId || '').trim();
  return normalizedNodeId === 'post-login-phone-verification' && isAddPhoneAuthFailure(error);
}
...
function resolveNodeFailureLogLevel(error, nodeId = '') {
  return isPostLoginPhoneVerificationAddPhoneWarning(error, nodeId)
    ? 'warn'
    : 'error';
}

function shouldSuppressNodeFailureLogInAutoRun(error, nodeId = '', state = {}) {
  if (resolveNodeFailureLogLevel(error, nodeId) !== 'warn') {
    return false;
  }
  ...
  if (normalizedPlan.kind === AUTO_RUN_TIMER_KIND_SCHEDULED_START) {
    return {
      loopOptions: {
        ...
        importedAccounts: normalizedPlan.importedAccounts,
        startStep: normalizedPlan.startStep,
        endStep: normalizedPlan.endStep,
        authorizationEmail: normalizedPlan.authorizationEmail,
      },
      statusPayload: {
        currentRun: 0,
        totalRuns: normalizedPlan.totalRuns,
  ...
  try {
    ...
    await broadcastAutoRunStatus(
      ...
      resumeOptions.statusPayload,
      {
        ...
        importedAccounts: plan.importedAccounts,
        startStep: plan.startStep,
        endStep: plan.endStep,
        authorizationEmail: plan.authorizationEmail,
        autoRunTimerPlan: null,
        scheduledAutoRunPlan: null,
      }
    );
...
async function scheduleAutoRun(totalRuns, options = {}) {
  ...
  if (delayMinutes <= 0) {
    await setState({
      ...
      executionMode: normalizeAutoRunExecutionMode(options.executionMode),
      importedAccounts: normalizeImportedAccounts(options.importedAccounts),
      startStep: normalizeAutoRunStepBoundary(options.startStep, PERSISTED_SETTING_DEFAULTS.startStep),
      endStep: normalizeAutoRunStepBoundary(options.endStep, PERSISTED_SETTING_DEFAULTS.endStep),
      authorizationEmail: String(options.authorizationEmail || '').trim(),
    });
    await addLog('自动运行延迟为 0 分钟，立即开始执行。', 'info');
    startAutoRunLoop(totalRuns, {
      autoRunSkipFailures: Boolean(options.autoRunSkipFailures),
      mode: options.mode,
      executionMode: options.executionMode,
      importedAccounts: options.importedAccounts,
      startStep: options.startStep,
      endStep: options.endStep,
      authorizationEmail: options.authorizationEmail,
    });
  ...
  await persistAutoRunTimerPlan(timerPlan, {
    ...
    executionMode: timerPlan.executionMode,
    importedAccounts: timerPlan.importedAccounts,
    startStep: timerPlan.startStep,
    endStep: timerPlan.endStep,
    authorizationEmail: timerPlan.authorizationEmail,
  });
  await addLog(
...
async function handleStepData(step, payload) {
  ...
  switch (step) {
    ...
    case 2:
      ...
      if (!payload.email && (payload.accountIdentifierType || payload.accountIdentifier)) {
        await setState({
    ...
    case 8:
      ...
      if (payload.skipLoginVerificationStep) {
        const latestState = await getState();
        const step8NodeId = getNodeIdByStepForState(8, latestState);
        const step8Status = step8NodeId ? latestState.nodeStatuses?.[step8NodeId] : '';
        // Step8 自身判定“可跳过”时，同步改为 skipped，保持与步骤 3 一致的蓝色“跳过”展示。
        if (step8NodeId && (step8Status === 'completed' || step8Status === 'pending')) {
          await setNodeStatus(step8NodeId, 'skipped');
        }
      }
      break;
    case 9:
...
async function completeNodeFromBackground(nodeId, payload = {}) {
  ...
  const completionStep = typeof getStepIdByNodeIdForState === 'function'
    ? Number(getStepIdByNodeIdForState(normalizedNodeId, latestState))
    : 0;
  const completionStepPrefix = Number.isInteger(completionStep) && completionStep > 0
    ? `【步骤${completionStep}】`
    : '';
  await addLog(`${completionStepPrefix}已完成`, 'ok', { nodeId: normalizedNodeId });
...
async function finalizeDeferredNodeExecutionError(nodeId, error) {
  ...
  if (!shouldSuppressNodeFailureLogInAutoRun(errorMessage, normalizedNodeId, latestState)) {
    await addLog(
      formatNodeFailureLogLine(errorMessage, normalizedNodeId),
      resolveNodeFailureLogLevel(errorMessage, normalizedNodeId),
      { nodeId: normalizedNodeId }
    );
  }
  await appendManualAccountRunRecordIfNeeded(`node:${normalizedNodeId}:failed`, latestState, errorMessage);
}
...
async function completeStep5FromTabUrlAfterTransportError(sourceError = null) {
  let signupTabId = await getTabId('openai-auth').catch(() => null);
  if (!Number.isInteger(signupTabId)) {
    signupTabId = await getTabId('signup-page').catch(() => null);
  }
  if (!Number.isInteger(signupTabId)) {
    return null;
  }
  ...
  await addLog(
    `步骤 5 [调试] 内容脚本通信中断，但后台确认标签页已进入放行页，按提交成功收尾。原始错误：${getErrorMessage(sourceError)}`,
    'warn',
...
async function executeNode(nodeId, options = {}) {
  ...
  try {
    ...
    const stepPrefix = Number.isInteger(step) && step > 0 ? `【步骤${step}】` : '';
    await addLog(`${stepPrefix}开始执行`, 'info', { nodeId: normalizedNodeId });
    ...
    if (!(deferRetryableTransportError && doesNodeUseCompletionSignal(normalizedNodeId, errorState) && isRetryableContentScriptTransportError(err))) {
      await setNodeStatus(normalizedNodeId, 'failed');
      if (!shouldSuppressNodeFailureLogInAutoRun(errorMessage, normalizedNodeId, errorState)) {
        await addLog(
          formatNodeFailureLogLine(errorMessage, normalizedNodeId),
          resolveNodeFailureLogLevel(errorMessage, normalizedNodeId),
          { nodeId: normalizedNodeId }
      ...
      await appendManualAccountRunRecordIfNeeded(`node:${normalizedNodeId}:failed`, errorState, errorMessage);
    } else {
      console.warn(
        LOG_PREFIX,
        `[executeNode] deferring retryable transport error for node ${normalizedNodeId}: ${errorMessage}`
      );
    }
    throw err;
  } finally {
...
const AUTO_RUN_NODE_DELAYS = Object.freeze({
  ...
  'post-login-phone-verification': 1000,
  'confirm-oauth': 1000,
  'platform-verify': 0,
});
...
async function clearAndBroadcastAccountRunHistory(stateOverride = null, options = {}) {
  if (!accountRunHistoryHelpers?.clearAccountRunHistory) {
    return { clearedCount: 0 };
  }

  const result = await accountRunHistoryHelpers.clearAccountRunHistory(stateOverride, options);
  await broadcastAccountRunHistoryUpdate();
  return result;
}
...
const autoRunController = self.MultiPageBackgroundAutoRunController?.createAutoRunController({
  ...
  isGptmailApiKeyUsageLimitError,
...
async function ensureAutoEmailReady(targetRun, totalRuns, attemptRuns) {
  ...
  if (isGptmailProvider(currentState)) {
    const email = await ensureGptmailAddressForFlow();
    await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮：GPTMail 邮箱已就绪：${email}（第 ${attemptRuns} 次尝试）===`, 'ok');
    return email;
  }
...
function resolveWorkflowBoundaryNodeIdByStep(step, state = {}, options = {}) {
  const prefer = String(options.prefer || 'start').trim().toLowerCase() === 'end' ? 'end' : 'start';
  if (typeof getStepIdsForState !== 'function' || typeof getNodeIdByStepForState !== 'function') {
    return '';
  }
  const stepIds = getStepIdsForState(state)
    .map((item) => normalizeAutoRunStepBoundary(item, 0))
    .filter((item) => item > 0)
    .sort((left, right) => left - right);
  if (!stepIds.length) {
    return '';
  ...
  const targetStep = normalizeAutoRunStepBoundary(step, 0);
  let resolvedStep = stepIds[0];
  if (prefer === 'start') {
    resolvedStep = stepIds.find((item) => item >= targetStep) || stepIds[stepIds.length - 1];
  } else {
    for (let index = stepIds.length - 1; index >= 0; index -= 1) {
      const candidate = stepIds[index];
  ...
  return String(getNodeIdByStepForState(resolvedStep, state) || '').trim();
}

function resolveAutoRunExecutionContext(context = {}, state = {}) {
  const executionMode = normalizeAutoRunExecutionMode(context?.executionMode ?? state?.executionMode);
  const importedAccounts = normalizeImportedAccounts(
    Array.isArray(context?.importedAccounts) ? context.importedAccounts : state?.importedAccounts
  );
  const executionStepRange = normalizeAutoRunStepRange(
    executionMode,
    context?.startStep ?? state?.startStep,
    context?.endStep ?? state?.endStep
  );
  const authorizationEmail = String(context?.authorizationEmail || state?.authorizationEmail || '').trim();
  return {
    executionMode,
    importedAccounts,
    startStep: executionStepRange.startStep,
...
async function runAutoSequenceFromNode(startNodeId, context = {}) {
  const state = await getState();
  const resolveExecutionContext = typeof resolveAutoRunExecutionContext === 'function'
    ? resolveAutoRunExecutionContext
    : ((rawContext = {}, rawState = {}) => {
      const modeRaw = String(rawContext?.executionMode ?? rawState?.executionMode ?? '').trim().toLowerCase();
      const executionMode = modeRaw === 'register_gpt'
        ? 'register_gpt'
        : (modeRaw === 'authorize_codex' ? 'authorize_codex' : 'full');
      const importedAccounts = Array.isArray(rawContext?.importedAccounts)
        ? rawContext.importedAccounts.map((item) => String(item || '').trim()).filter(Boolean)
        : (Array.isArray(rawState?.importedAccounts)
          ? rawState.importedAccounts.map((item) => String(item || '').trim()).filter(Boolean)
          : []);
      const defaultRange = executionMode === 'register_gpt'
        ? { startStep: 1, endStep: 6 }
        : (executionMode === 'authorize_codex' ? { startStep: 7, endStep: 11 } : { startStep: 1, endStep: 11 });
      let startStep = Math.max(1, Math.floor(Number(rawContext?.startStep ?? rawState?.startStep) || defaultRange.startStep));
      let endStep = Math.max(1, Math.floor(Number(rawContext?.endStep ?? rawState?.endStep) || defaultRange.endStep));
      if (startStep > endStep) {
        startStep = defaultRange.startStep;
        endStep = defaultRange.endStep;
      }
      return {
        executionMode,
        importedAccounts,
        startStep,
        endStep,
        authorizationEmail: String(rawContext?.authorizationEmail || rawState?.authorizationEmail || '').trim(),
      };
    });
  const resolveBoundaryNodeByStep = typeof resolveWorkflowBoundaryNodeIdByStep === 'function'
    ? resolveWorkflowBoundaryNodeIdByStep
    : ((step, rawState = {}, options = {}) => {
      const prefer = String(options.prefer || 'start').trim().toLowerCase() === 'end' ? 'end' : 'start';
      if (typeof getStepIdsForState !== 'function' || typeof getNodeIdByStepForState !== 'function') {
        return '';
      }
      const stepIds = getStepIdsForState(rawState)
      ...
      const targetStep = Math.max(0, Math.floor(Number(step) || 0));
      let resolvedStep = stepIds[0];
      if (prefer === 'start') {
        resolvedStep = stepIds.find((item) => item >= targetStep) || stepIds[stepIds.length - 1];
      } else {
      ...
      return String(getNodeIdByStepForState(resolvedStep, rawState) || '').trim();
    });
  const executionContext = resolveExecutionContext(context, state);
  let normalizedStartNodeId = String(startNodeId || '').trim();
  const hasExecutionRangeInContext = Object.prototype.hasOwnProperty.call(context || {}, 'executionMode')
    || Object.prototype.hasOwnProperty.call(context || {}, 'startStep')
    || Object.prototype.hasOwnProperty.call(context || {}, 'endStep');
  if (!context?.continued && hasExecutionRangeInContext) {
    const boundaryStartNodeId = resolveBoundaryNodeByStep(executionContext.startStep, state, {
      prefer: 'start',
    });
    if (boundaryStartNodeId) {
      normalizedStartNodeId = boundaryStartNodeId;
    }
  }
  ...
  return runAutoSequenceFromNodeGraph(normalizedStartNodeId, {
    ...context,
    ...executionContext,
  });
}
...
async function runAutoSequenceFromNodeGraph(startNodeId, context = {}) {
  ...
  const executionContext = resolveExecutionContext(context, initialState);
  const executionMode = executionContext.executionMode;
  const executionModeRegisterGpt = (typeof AUTO_RUN_EXECUTION_MODE_REGISTER_GPT === 'string' && AUTO_RUN_EXECUTION_MODE_REGISTER_GPT.trim())
    ? AUTO_RUN_EXECUTION_MODE_REGISTER_GPT
    : 'register_gpt';
  const executionModeAuthorizeCodex = (typeof AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX === 'string' && AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX.trim())
    ? AUTO_RUN_EXECUTION_MODE_AUTHORIZE_CODEX
    : 'authorize_codex';
  const importedAccounts = executionContext.importedAccounts;
  const executionEndStep = executionContext.endStep;
  const resolveExecutionEndNodeIndex = (state = {}, nodeIds = getAutoRunWorkflowNodeIds(state)) => {
    const boundaryNodeId = resolveBoundaryNodeByStep(executionEndStep, state, { prefer: 'end' });
    if (!boundaryNodeId) {
      return nodeIds.length - 1;
    }
    const boundaryNodeIndex = nodeIds.indexOf(boundaryNodeId);
    return boundaryNodeIndex >= 0 ? boundaryNodeIndex : nodeIds.length - 1;
  };
  const ensureAuthorizationEmailForCurrentRun = async () => {
    if (executionMode !== executionModeAuthorizeCodex) {
      return '';
    }
    const targetIndex = Math.max(0, Math.floor(Number(targetRun) || 0) - 1);
    const selectedEmail = String(importedAccounts[targetIndex] || '').trim();
    if (!selectedEmail) {
      throw new Error(`授权Codex流程第 ${targetRun} 轮缺少导入账号，请检查“运行次数”与导入账号数量。`);
    }
    if (!isBasicEmailFormat(selectedEmail)) {
      throw new Error(`授权Codex流程第 ${targetRun} 轮邮箱格式无效：${selectedEmail}`);
    }
    await setEmailStateSilently(selectedEmail);
    await setState({
      // 授权阶段（7-11）本轮使用的邮箱。
      authorizationEmail: selectedEmail,
      forceLoginIdentifierType: 'email',
      forceEmailLogin: true,
      signupMethod: SIGNUP_METHOD_EMAIL,
      resolvedSignupMethod: SIGNUP_METHOD_EMAIL,
      step8VerificationTargetEmail: selectedEmail,
    });
  ...
  while (true) {
  if (executionMode === executionModeAuthorizeCodex) {
    await ensureAuthorizationEmailForCurrentRun();
  }
  ...
  if (continueCurrentAttempt) {
    ...
    if (executionMode === executionModeAuthorizeCodex) {
      await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮：第 ${attemptRuns} 次尝试，仅执行授权链路（步骤 ${executionContext.startStep}-${executionContext.endStep}）===`, 'info');
    } else if (executionMode === executionModeRegisterGpt) {
      await addLog(`=== 目标 ${targetRun}/${totalRuns} 轮：第 ${attemptRuns} 次尝试，仅执行注册链路（步骤 ${executionContext.startStep}-${executionContext.endStep}）===`, 'info');
    } else {
  ...
  while (nodeIndex < nodeIds.length) {
    ...
    const maxExecutionNodeIndex = resolveExecutionEndNodeIndex(latestState, nodeIds);
    if (nodeIndex > maxExecutionNodeIndex) {
      break;
    }
    ...
    try {
      ...
      if (nodeId === 'fetch-signup-code') {
        ...
        if (typeof isGptmailApiKeyUsageLimitError === 'function' && isGptmailApiKeyUsageLimitError(err)) {
          throw err;
        }
      ...
      if (nodeId === 'fill-profile' && isSignupUserAlreadyExistsFailure(err)) {
        if (executionMode === executionModeRegisterGpt) {
          // register_gpt：命中已注册账号后直接结束当前轮，交由外层轮询进入下一轮。
          await addLog('节点 fill-profile：步骤 5 检测到 user_already_exists（账号已注册过），注册流程当前轮将结束并进入下一轮。', 'warn');
          throw err;
        }

        const transitionState = await getState();
        const step5Status = getNodeStatusForNode(transitionState, nodeId);
        if (step5Status !== 'running' && step5Status !== 'completed' && step5Status !== 'manual_completed') {
          await setNodeStatus(nodeId, 'skipped');
        }

        const waitRegistrationStep = typeof getStepIdByKeyForState === 'function'
          ? getStepIdByKeyForState('wait-registration-success', transitionState)
          : null;
        const waitRegistrationNodeId = (() => {
          if (Number.isInteger(waitRegistrationStep) && waitRegistrationStep > 0) {
            return String(getNodeIdByStepForState(waitRegistrationStep, transitionState) || '').trim();
          }
          return String(getNodeIdByStepForState(6, transitionState) || 'wait-registration-success').trim();
        })();
        if (waitRegistrationNodeId) {
          const step6Status = getNodeStatusForNode(await getState(), waitRegistrationNodeId);
          if (step6Status !== 'running' && step6Status !== 'completed' && step6Status !== 'manual_completed') {
            await setNodeStatus(waitRegistrationNodeId, 'skipped');
          }
        }

        await addLog(
          `节点 ${getNodeLabel(nodeId, latestState)}：步骤 5 检测到 user_already_exists（账号已注册过），已按步骤 3 跳转逻辑跳过步骤 5/6，流程将直接进入步骤 7。`,
          'warn'
        );
        nodeIndex += 1;
        continue;
      ...
      if (restartDecision.blockedByAddPhone) {
        // add-phone 分支仅用于阻止自动重开；这里不再额外输出警告日志。
      }
      throw err;
    }
  }
...
function getMailConfig(state) {
  ...
  const gptmailProvider = typeof GPTMAIL_PROVIDER !== 'undefined' ? GPTMAIL_PROVIDER : 'gptmail';
  if (provider === gptmailProvider) {
    return { provider: gptmailProvider, label: 'GPTMail' };
  }
...
async function refreshOAuthUrlBeforeStep6(state, options = {}) {
  ...
  await addLog(`走普通 CPA / SUB2API / Codex2API 链路（当前面板：${getPanelModeLabel(state)}），正在刷新 OAuth 登录地址...`, 'info', {
    step: visibleStep,
    stepKey: 'oauth-login',
  });
...
async function getPostStep6AutoRestartDecision(step, error) {
  ...
  if (typeof isGptmailApiKeyUsageLimitError === 'function' && isGptmailApiKeyUsageLimitError(error)) {
    return {
      shouldRestart: false,
...
// 统一记录 Step5 提交后复核链路的调试信息，方便定位导航/页面状态分歧。
async function addStep5PostCompletionDebugLog(message, details = {}) {
  const summary = [
    ...
    details?.navigationStarted !== undefined ? `navigationStarted=${Boolean(details.navigationStarted)}` : null,
    details?.tabUrl ? `tabUrl=${details.tabUrl}` : null,
    details?.pageState?.url ? `contentUrl=${details.pageState.url}` : null,
    details?.pageState ? `retryPage=${Boolean(details.pageState.retryPage)}` : null,
    details?.pageState ? `retryEnabled=${Boolean(details.pageState.retryEnabled)}` : null,
    details?.pageState ? `successState=${details.pageState.successState || 'none'}` : null,
    details?.pageState ? `profileVisible=${Boolean(details.pageState.profileVisible)}` : null,
    details?.pageState ? `unknownAuthPage=${Boolean(details.pageState.unknownAuthPage)}` : null,
    details?.pageState ? `maxCheckAttemptsBlocked=${Boolean(details.pageState.maxCheckAttemptsBlocked)}` : null,
    details?.pageState ? `userAlreadyExistsBlocked=${Boolean(details.pageState.userAlreadyExistsBlocked)}` : null,
    details?.pageState?.errorText ? `errorText=${details.pageState.errorText}` : null,
  ]
    .filter(Boolean)
    .join(' | ');
...
async function validateStep5PostCompletion(tabId, completionPayload = {}) {
  ...
  while (true) {
    ...
    if (currentUrl && isStep5CompletionChatgptUrl(currentUrl)) {
      await debugLog('后台直接通过标签页 URL 确认已进入放行页，步骤 5 完成。', {
        completionOutcome: String(completionPayload?.outcome || '').trim(),
        completionUrl: String(completionPayload?.url || '').trim(),
        navigationStarted: Boolean(completionPayload?.navigationStarted),
        tabUrl: currentUrl,
    ...
    if (pageState.successState) {
      const successUrl = String(pageState.url || currentUrl || '').trim();
      if (successUrl && isStep5CompletionChatgptUrl(successUrl)) {
      ...
      await debugLog('后台复核发现非放行页的成功候选，按未完成处理。', {
        completionOutcome: String(completionPayload?.outcome || '').trim(),
        completionUrl: String(completionPayload?.url || '').trim(),
        navigationStarted: Boolean(completionPayload?.navigationStarted),
        tabUrl: currentUrl,
      ...
      throw new Error(`步骤 5：资料提交后尚未跳转到放行页面（chatgpt.com 或 passkey enroll），不能标记完成。当前状态：${pageState.successState}，URL: ${pageState.url || currentUrl || 'unknown'}`);
    }
    ...
    if (pageState.unknownAuthPage) {
      await debugLog('后台复核进入未识别认证页，无法确认成功。', {
        completionOutcome: String(completionPayload?.outcome || '').trim(),
        completionUrl: String(completionPayload?.url || '').trim(),
        navigationStarted: Boolean(completionPayload?.navigationStarted),
        tabUrl: currentUrl,
...
async function rerunStep7ForStep8Recovery(options = {}) {
  ...
  const authLoginStepPrefix = Number.isInteger(authLoginStep) && authLoginStep > 0
    ? `【步骤${authLoginStep}】`
    : '';
  await addLog(`${authLoginStepPrefix}开始执行`, 'info', { nodeId: authLoginNodeId });
  ...
  try {
    ...
    await addLog(formatNodeFailureLogLine(errorMessage, authLoginNodeId), 'error', { nodeId: authLoginNodeId });
    await appendManualAccountRunRecordIfNeeded(`node:${authLoginNodeId}:failed`, latestState, errorMessage);
    throw err;
  }
