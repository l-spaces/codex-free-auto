...
test('step 4 does not request a fresh code first for YYDS Mail', async () => {
  let capturedOptions = null;
...
test('step 4 does not open a mail tab for YYDS Mail provider', async () => {
  const tabReuses = [];
  let resolveCalls = 0;
...
test('step 4 skips opening mail tab when source or url is missing', async () => {
  const tabReuses = [];
  const logs = [];
  ...
  assert.deepStrictEqual(tabReuses, []);
  assert.equal(
    logs.some((entry) => /缺少.*source.*url/.test(String(entry.message || ''))),
    true
  );
});
