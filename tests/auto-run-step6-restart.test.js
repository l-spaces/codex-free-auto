...
test('auto-run stops restarting once add-phone is detected', async () => {
  ...
  assert.ok(!result.events.logs.some(({ message }) => /回到节点 auth-login 重新开始授权流程/.test(message)));
});
...
test('auto-run does not restart login chain when GPTMail API key usage limit is reached', async () => {
  const harness = createHarness({
    failureStep: 8,
    failureBudget: 1,
    failureMessage: '步骤 8：GPTMail 轮询失败：GPTMail 请求失败：API key usage limit reached',
    authState: { state: 'password_page', url: 'https://auth.openai.com/log-in' },
    customState: {
  ...
  assert.match(result?.error?.message || '', /API key usage limit reached/);
  assert.equal(result.events.invalidations.length, 0);
  assert.deepStrictEqual(result.events.steps, [7, 8]);
  assert.ok(!result.events.logs.some(({ message }) => /回到节点 auth-login 重新开始授权流程/.test(message)));
});
...
test('auto-run does not restart step 7 when phone verification exhausted replacement attempts in add-phone flow', async () => {
  const harness = createHarness({
    failureStep: 9,
    failureBudget: 1,
    failureMessage: 'Step 9: phone verification did not succeed after 3 attempts.',
    authState: { state: 'add_phone_page', url: 'https://auth.openai.com/add-phone' },
  });
