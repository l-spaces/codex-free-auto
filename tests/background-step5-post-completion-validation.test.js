...
test('step 5 post-completion validation allows direct passkey enroll landing', async () => {
  const api = new Function(`
const messages = [];
...
const chrome = {
  tabs: {
    async get() {
      return { url: 'https://auth.openai.com/create-account-enroll-passkey' };
    },
  },
};
...
`)();
  ...
  assert.equal(result.successState, 'logged_in_home');
  assert.equal(result.url, 'https://auth.openai.com/create-account-enroll-passkey');
  assert.deepStrictEqual(snapshot.messages, []);
});

test('step 5 recovers from transport close when tab already reached allowed completion page', async () => {
  const api = new Function(`
const logs = [];
let waitCalls = 0;
...
`)();
  ...
  assert.equal(
    snapshot.logs.some(({ message }) => /通信中断，但后台确认标签页已进入/.test(message)),
    true
  );
});
