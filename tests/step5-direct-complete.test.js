...
test('step 5 uses adaptive early retry interval before max retry interval while profile is still visible', async () => {
  const api = new Function(`
const realDateNow = Date.now;
let now = 0;
const clicks = [];
...
return {
  run() {
    return waitForStep5SubmitOutcome({ timeoutMs: 1800, retryClickIntervalMs: 3500 });
  },
  snapshot() {
    return { clicks, now };
  },
...
test('step 5 navigation reporter hands off completion on beforeunload', () => {
  const api = new Function(`
const events = [];
...
test('step 5 navigation reporter hands off completion on pagehide', () => {
  const api = new Function(`
const events = [];
const listeners = new Map();
const location = {
...
function isLikelyLoggedInChatgptHomeUrl() { return false; }
function isStep5CompletionChatgptUrl() { return false; }
function isOAuthConsentPage() { return false; }
function isAddPhonePageReady() { return false; }
...
test('step 5 treats passkey enroll page as success', () => {
  const api = new Function(`
const location = {
