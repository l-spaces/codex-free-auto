...
test('verification flow uses current GPTMail address as step 4 poll target', () => {
  const helpers = createVerificationFlowTestHelpers();
  ...
  assert.equal(payload.targetEmail, 'fresh@baileybridge.org');
});
...
test('verification flow keeps step 8 successful and marks phone verification required when add-phone appears after login code submit', async () => {
  const events = [];
...
test('verification flow no longer throws fatal add-phone on manual step 8 confirmation and keeps step handoff', async () => {
  const events = [];
  const helpers = api.createVerificationFlowHelpers({
    addLog: async () => {},
    chrome: {
    ...
    setStepStatus: async (step, status) => {
      events.push(['status', step, status]);
    },
    sleepWithStop: async () => {},
    throwIfStopped: () => {},
    VERIFICATION_POLL_MAX_ROUNDS: 5,
  ...
  assert.equal(events.some((entry) => entry[0] === 'complete'), false);
  assert.equal(
    events.some((entry) => entry[0] === 'status' && entry[2] === 'skipped'),
    true
  );
});
...
test('verification flow caps oversized 2925 polling plan rounds to provider max attempts', async () => {
  const pollMaxAttempts = [];
  let pollCalls = 0;
  ...
  const helpers = api.createVerificationFlowHelpers({
    ...
    sendToContentScript: async (_source, message) => {
      ...
      return message.type === 'FILL_CODE' ? { success: true } : {};
    },
    sendToContentScriptResilient: async () => ({}),
    sendToMailContentScriptResilient: async (_mail, message) => {
      if (message.type !== 'POLL_EMAIL') {
  ...
  await helpers.resolveVerificationStep(
    ...
    {
      maxResendRequests: 2,
      initialPollMaxAttempts: 20,
      pollAttemptPlan: [2, 30, 999],
      requestFreshCodeFirst: false,
      filterAfterTimestamp: 123,
      resendIntervalMs: 0,
    }
  ...
  assert.deepStrictEqual(pollMaxAttempts.slice(0, 3), [2, 15, 15]);
});

test('verification flow caps oversized 2925 initial polling attempts to provider max attempts', async () => {
  const pollMaxAttempts = [];
  ...
  const helpers = api.createVerificationFlowHelpers({
    ...
    pollLuckmailVerificationCode: async () => ({}),
    sendToContentScript: async (_source, message) => (
      message.type === 'FILL_CODE' ? { success: true } : {}
    ),
    sendToContentScriptResilient: async () => ({}),
    sendToMailContentScriptResilient: async (_mail, message) => {
      if (message.type !== 'POLL_EMAIL') {
  ...
  assert.deepStrictEqual(pollMaxAttempts, [15]);
});
...
test('verification flow avoids resend storms when iCloud polling keeps hitting transport errors', async () => {
  ...
  assert.equal(
    logs.filter(({ message }) => /did not respond in 1s/i.test(message)).length <= 2,
    true
  );
});
