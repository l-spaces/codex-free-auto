...
test('signup flow helper falls back to auth-state probe when post-identity URL wait times out', async () => {
  const calls = {
    probeMessages: 0,
  };
  ...
  const helpers = signupFlowApi.createSignupFlowHelpers({
    addLog: async () => {},
    buildGeneratedAliasEmail: () => 'demo@example.com',
    chrome: {
      tabs: {
        get: async () => ({ id: 9, url: 'https://auth.openai.com/u/signup/identifier' }),
      },
    },
    ensureContentScriptReadyOnTab: async () => {},
    ensureHotmailAccountForFlow: async () => ({}),
    ...
    isHotmailProvider: () => false,
    isLuckmailProvider: () => false,
    isSignupEmailVerificationPageUrl: (url) => /email-verification/i.test(String(url || '')),
    isSignupPasswordPageUrl: (url) => /create-account\/password/i.test(String(url || '')),
    reuseOrCreateTab: async () => 9,
    sendToContentScriptResilient: async (_target, message) => {
      if (message?.type === 'GET_LOGIN_AUTH_STATE') {
        calls.probeMessages += 1;
        return {
  ...
  const result = await helpers.ensureSignupPostEmailPageReadyInTab(9, 2, {
    postIdentityProbeTimeoutMs: 100,
  });

  assert.equal(result.ready, true);
  assert.equal(result.state, 'verification_page');
  assert.equal(calls.probeMessages > 0, true);
});

test('signup flow helper keeps timeout error when url wait and auth-state probe both fail', async () => {
  const helpers = signupFlowApi.createSignupFlowHelpers({
    addLog: async () => {},
    buildGeneratedAliasEmail: () => 'demo@example.com',
    chrome: {
  ...
  await assert.rejects(
    () => helpers.ensureSignupPostEmailPageReadyInTab(9, 2, { postIdentityProbeTimeoutMs: 20 }),
    /等待注册身份提交后的页面跳转超时/
  );
});
