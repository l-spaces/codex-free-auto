...
test('account run history helper can clear and sync snapshot without adding a log entry', async () => {
  const source = fs.readFileSync('background/account-run-history.js', 'utf8');
  const globalScope = {};
  const api = new Function('self', `${source}; return self.MultiPageBackgroundAccountRunHistory;`)(globalScope);

  let storedHistory = [{
    recordId: 'silent@example.com',
  ...
  global.fetch = async (url, options = {}) => {
    ...
    return {
      ok: true,
      json: async () => ({ ok: true, filePath: 'C:/tmp/account-run-history.json' }),
    };
  };
  ...
  const clearResult = await helpers.clearAccountRunHistory(null, { suppressLog: true });

  assert.deepStrictEqual(clearResult, { clearedCount: 1 });
  assert.deepStrictEqual(storedHistory, []);
  assert.equal(fetchCalls.length, 1);
  assert.equal(fetchCalls[0].url, 'http://127.0.0.1:17373/sync-account-run-records');
  assert.deepStrictEqual(JSON.parse(fetchCalls[0].options.body).records, []);
