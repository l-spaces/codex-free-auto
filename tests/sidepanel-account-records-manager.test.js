...
test('sidepanel reset clears visible account records without sending a second clear request', () => {
  const resetStart = sidepanelSource.indexOf("btnReset.addEventListener('click'");
  const resetEnd = sidepanelSource.indexOf('// Clear log', resetStart);
  const resetBlock = sidepanelSource.slice(resetStart, resetEnd);

  assert.notEqual(resetStart, -1, 'missing reset click handler');
  assert.match(resetBlock, /type:\s*'RESET'/);
  assert.match(resetBlock, /accountRunHistory:\s*\[\]/);
  assert.match(resetBlock, /logArea\.innerHTML\s*=\s*''/);
  assert.doesNotMatch(resetBlock, /CLEAR_ACCOUNT_RUN_HISTORY/);
});
