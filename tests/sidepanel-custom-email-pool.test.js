...
test('sidepanel html exposes custom email pool generator option and input row', () => {
  ...
  assert.match(html, /id="row-duck-api-authorization"/);
  assert.match(html, /id="input-duck-api-authorization"/);
});
...
test('sidepanel persists duck authorization and toggles its row for duck generator', () => {
  const collectPayloadSource = extractFunction('collectSettingsPayload');
  const updateMailProviderUiSource = extractFunction('updateMailProviderUI');

  assert.match(
    collectPayloadSource,
    /duckApiAuthorization:\s*typeof inputDuckApiAuthorization !== 'undefined' && inputDuckApiAuthorization[\s\S]*String\(inputDuckApiAuthorization\.value \|\| ''\)\.trim\(\)/,
  );
  assert.match(updateMailProviderUiSource, /const useDuckGenerator = selectedGenerator === 'duck';/);
  assert.match(updateMailProviderUiSource, /const showDuckApiAuthorization = useEmailGenerator && useDuckGenerator;/);
  assert.match(
    updateMailProviderUiSource,
    /if \(typeof rowDuckApiAuthorization !== 'undefined' && rowDuckApiAuthorization\) \{\s*rowDuckApiAuthorization\.style\.display = showDuckApiAuthorization \? '' : 'none';\s*\}/,
  );
});

test('sidepanel avoids overwriting duck authorization input while user is actively editing', () => {
  ...
  assert.match(
    applySettingsStateSource,
    /if \(!isEditingDuckAuthorization \|\| !settingsDirty\) \{\s*inputDuckApiAuthorization\.value = nextDuckAuthorization;\s*\}/,
  );
});
