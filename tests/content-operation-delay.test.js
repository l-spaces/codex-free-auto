...
test('operation delay runs action first and waits once after completion', async () => {
  ...
  assert.deepStrictEqual(events, ['operation:start', 'operation:end', 'sleep:1200']);
});
...
test('operation delay defaults unresolved settings to enabled', async () => {
  ...
  assert.deepStrictEqual(events, ['operation', 'sleep:1200']);
});
...
test('grouped split-code metadata still delays only once', async () => {
  ...
  assert.deepStrictEqual(events, ['fill:0', 'fill:1', 'fill:2', 'fill:3', 'fill:4', 'fill:5', 'sleep:1400']);
});

test('operation delay maps delay by operation kind', () => {
  const api = loadOperationDelayApi();
  assert.equal(api.resolveOperationDelayMs({ kind: 'click' }), 1000);
  assert.equal(api.resolveOperationDelayMs({ kind: 'submit' }), 1300);
  assert.equal(api.resolveOperationDelayMs({ kind: 'hidden-sync' }), 300);
  assert.equal(api.resolveOperationDelayMs({ kind: 'unknown-kind' }), 2000);
});
