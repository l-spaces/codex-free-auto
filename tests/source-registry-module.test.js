...
test('shared source registry exposes canonical source, alias, detection, and ready policies', () => {
  ...
  assert.equal(registry.driverAcceptsCommand('openai-auth', 'post-login-phone-verification'), false);
  assert.equal(registry.driverAcceptsCommand('openai-auth', 'bind-email'), false);
  assert.equal(registry.driverAcceptsCommand('openai-auth', 'fetch-bind-email-code'), false);
