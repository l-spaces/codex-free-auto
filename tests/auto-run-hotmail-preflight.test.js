...
test('auto-run controller appends success record when register_gpt round completes', async () => {
  const events = {
    accountRecords: [],
    runContexts: [],
  };
  ...
  let currentState = {
    ...
    sourceLastUrls: {},
    autoRunRoundSummaries: [],
    email: 'register-gpt@example.com',
    accountIdentifierType: 'email',
    accountIdentifier: 'register-gpt@example.com',
  };
  ...
  const controller = api.createAutoRunController({
    ...
    runAutoSequenceFromStep: async (_step, context = {}) => {
      events.runContexts.push({ ...context });
    },
    runtime,
    setState: async (updates = {}) => {
      currentState = {
  ...
  assert.equal(events.runContexts.length, 1);
  assert.equal(events.runContexts[0].executionMode, 'register_gpt');
  assert.equal(events.runContexts[0].startStep, 1);
  assert.equal(events.runContexts[0].endStep, 6);
  assert.deepEqual(events.accountRecords, [{
    status: 'success',
    reason: '注册GPT流程完成',
  }]);
});
