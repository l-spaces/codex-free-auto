...
test('resetState preserves LuckMail session config, used map, and preserve tag cache while clearing runtime purchase state', async () => {
  ...
  const factory = new Function([
    ...
    "          reusablePhoneActivation: { activationId: 'rx-001', phoneNumber: '66951112222', provider: 'legacy-provider', serviceCode: 'dr', countryId: 52, successfulUses: 1, maxUses: 3 },",
  ...
  assert.equal(Object.prototype.hasOwnProperty.call(snapshot.storedPayload, 'reusablePhoneActivation'), true);
  assert.equal(snapshot.storedPayload.reusablePhoneActivation, null);
  assert.equal(Object.prototype.hasOwnProperty.call(snapshot.storedPayload, 'currentPhoneActivation'), true);
