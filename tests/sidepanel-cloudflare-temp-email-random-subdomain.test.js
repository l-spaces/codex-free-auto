...
} = require('../utils/mail-provider-utils');
...
const inputTempEmailFixedMailbox = { value: '' };
...
return {
  ...
  api.applyCloudflareTempEmailSettingsState({
    ...
    cloudflareTempEmailFixedMailbox: 'fixed@example.com',
  ...
  assert.equal(api.inputTempEmailFixedMailbox.value, 'fixed@example.com');
...
const rowTempEmailFixedMailbox = ${JSON.stringify(createRow('none'))};
...
return {
  ...
  assert.equal(api.rowTempEmailFixedMailbox.style.display, 'none');
  ...
  api.updateMailProviderUI();
  assert.equal(api.rowTempEmailDomain.style.display, '');
  assert.equal(api.rowTempEmailFixedMailbox.style.display, '');
});
