...
function ensureCloudflareTempEmailConfig() {
  return {
    ...
    fixedMailbox: options.fixedMailbox,
...
test('pollCloudflareTempEmailVerificationCode uses fixed mailbox for generated Cloudflare Temp Email lookup', async () => {
  const api = createProviderApi({
    fixedMailbox: 'fixed@email.20021108.xyz',
    messages: [{
      id: 'mail-fixed',
      address: 'fixed@email.20021108.xyz',
  ...
  const result = await api.pollCloudflareTempEmailVerificationCode(4, {
    ...
    mailProvider: 'cloudflare-temp-email',
    emailGenerator: 'cloudflare-temp-email',
    cloudflareTempEmailFixedMailbox: 'fixed@email.20021108.xyz',
  }, {
    targetEmail: 'generated@email.20021108.xyz',
    maxAttempts: 1,
    intervalMs: 1,
  ...
  assert.deepEqual(api.snapshot().listCalls.map((call) => call.address), ['fixed@email.20021108.xyz']);
});
