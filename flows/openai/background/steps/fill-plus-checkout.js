  ...
  function createPlusCheckoutBillingExecutor(deps = {}) {
    ...
    function resolveBillingAddressCountry(state = {}, countryOverride = '', paymentMethod = PLUS_PAYMENT_METHOD_PAYPAL) {
      ...
      if (normalizedPaymentMethod === PLUS_PAYMENT_METHOD_GOPAY) {
        const countryCode = checkoutCountry || savedCheckoutCountry || 'ID';
        return {
          countryCode,
          requestedCountry: normalizeText(countryOverride)
            || normalizeText(state.plusCheckoutCountry)
            || 'ID',
          source: checkoutCountry ? 'checkout_page' : (savedCheckoutCountry ? 'checkout_state' : 'gopay_fallback'),
        };
      }

      const countryCode = checkoutCountry || savedCheckoutCountry || 'DE';
      return {
        countryCode,
        requestedCountry: normalizeText(countryOverride)
          || normalizeText(state.plusCheckoutCountry)
          || 'DE',
        source: checkoutCountry ? 'checkout_page' : (savedCheckoutCountry ? 'checkout_state' : 'paypal_fallback'),
      };
    }
