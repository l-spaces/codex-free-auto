...
- Network policy：网络保护、探测域名与失败边界应由 flow 明确声明，不在通用层写死。
...
- OpenAI 专属账号产物字段，如 OAuth state、平台验证字段。
...
3. 建立 `flows/openai` 边界，把 OpenAI 步骤、content driver、OAuth 和接码逻辑收拢进去。
4. 用第二个真实注册项目验证 flow registry 和 workflow engine，而不是靠假想接口设计过度抽象。
5. 稳定后清理旧 `step` 数字兼容层。

## 6. 新 flow 接入原则

新增注册项目时，只允许新增该 flow 的 workflow、content driver、mail rules、network profile 和必要的私有配置。不要直接修改 OpenAI 步骤，也不要把 OpenAI 的手机号验证、OAuth 逻辑提升为通用能力。
