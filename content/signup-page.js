...
async function resendVerificationCode(step, timeout = 45000) {
  ...
  while (Date.now() - start < timeout) {
    ...
    if (action && isActionEnabled(action)) {
      ...
      const resendClickUrl = String(location?.href || '');
      const resendClickText = getActionText(action);
      ...
      // Prefer condition-based settle: once page/url/button state changes, continue immediately.
      const settleStart = Date.now();
      while (Date.now() - settleStart < 1200) {
        throwIfStopped();
        if (is405MethodNotAllowedPage()) break;
        throwIfContactVerificationServerError();
        const nextAction = findResendVerificationCodeTrigger({ allowDisabled: true });
        if (String(location?.href || '') !== resendClickUrl) break;
        if (!nextAction || !isActionEnabled(nextAction)) break;
        if (getActionText(nextAction) !== resendClickText) break;
        await sleep(150);
      }
...
async function waitForSignupEntryState(options = {}) {
  ...
  const {
    ...
    entryRetryTuning = null,
  } = options;
  const normalizeTuningNumber = (value, fallback, min, max) => {
  ...
  const tuning = entryRetryTuning && typeof entryRetryTuning === 'object'
    ? entryRetryTuning
    : {};
  // Step2 等待/重试参数：支持从 payload.step2EntryRetryTuning 覆盖，未提供时使用默认值。
  const maxSignupEntryClickRetries = normalizeTuningNumber(
    tuning.maxSignupEntryClickRetries ?? tuning.maxRetries,
    5,
    0,
    12
  );
  const entryClickBaseDelayMs = normalizeTuningNumber(
    tuning.entryClickBaseDelayMs ?? tuning.baseDelayMs,
    1200,
    200,
    8000
  );
  const entryClickBackoffStepMs = normalizeTuningNumber(
    tuning.entryClickBackoffStepMs ?? tuning.backoffStepMs,
    600,
    0,
    4000
  );
  const entryClickMaxDelayMs = normalizeTuningNumber(
    tuning.entryClickMaxDelayMs ?? tuning.maxDelayMs,
    3600,
    entryClickBaseDelayMs,
    10000
  );
  const entryClickCooldownMs = normalizeTuningNumber(
    tuning.entryClickCooldownMs ?? tuning.cooldownMs,
    1200,
    100,
    5000
  );
  const switchToEmailCooldownMs = normalizeTuningNumber(
    tuning.switchToEmailCooldownMs ?? tuning.switchCooldownMs,
    1200,
    200,
    5000
  );
  const stateProbeThresholdMs = normalizeTuningNumber(
    tuning.stateProbeThresholdMs ?? tuning.probeThresholdMs,
    4000,
    500,
    Math.max(500, timeout)
  );
  const pollIntervalMs = normalizeTuningNumber(
    tuning.pollIntervalMs ?? tuning.statePollIntervalMs,
    200,
    80,
    1000
  );
  ...
  while (Date.now() - start < timeout) {
    ...
    if (snapshot.state === 'phone_entry') {
      ...
      if (snapshot.switchToEmailTrigger && Date.now() - lastSwitchToEmailAt >= switchToEmailCooldownMs) {
      ...
      if (logDiagnostics && !slowSnapshotLogged && Date.now() - start >= stateProbeThresholdMs) {
        slowSnapshotLogged = true;
        log(`步骤 ${step}：等待手机号入口切换超过 ${Math.floor(stateProbeThresholdMs / 1000)} 秒，页面诊断快照：${JSON.stringify(getSignupEntryDiagnostics())}`, 'warn');
      }
    ...
    if (snapshot.state === 'entry_home') {
      ...
      if (Date.now() - lastTriggerClickAt >= entryClickCooldownMs) {
        ...
        const entryClickDelayMs = Math.min(
          entryClickBaseDelayMs + retryAttempt * entryClickBackoffStepMs,
          entryClickMaxDelayMs
        );
        const entryClickDelaySeconds = (entryClickDelayMs / 1000).toFixed(entryClickDelayMs % 1000 === 0 ? 0 : 1);
        if (logDiagnostics) {
          log(`步骤 ${step}：正在点击官网注册入口（第 ${clickAttempts}/${maxSignupEntryClickAttempts} 次）："${getActionText(snapshot.signupTrigger).slice(0, 80)}"`);
        }
        log(retryAttempt > 0
          ? `步骤 ${step}：上次点击后仍未进入邮箱输入页，等待 ${entryClickDelaySeconds} 秒后重试点击官网注册入口（重试 ${retryAttempt}/${maxSignupEntryClickRetries}）...`
          : `步骤 ${step}：已找到官网注册入口，等待 ${entryClickDelaySeconds} 秒后点击...`);
        await sleep(entryClickDelayMs);
        throwIfStopped();
    ...
    if (logDiagnostics && !slowSnapshotLogged && Date.now() - start >= stateProbeThresholdMs) {
      slowSnapshotLogged = true;
      log(`步骤 ${step}：等待注册入口超过 ${Math.floor(stateProbeThresholdMs / 1000)} 秒，页面诊断快照`, 'warn');
    }
...
async function fillSignupEmailAndContinue(email, step, runtimeOptions = {}) {
  const performOperationWithDelay = typeof getOperationDelayRunner === 'function'
    ? getOperationDelayRunner()
    : async (metadata, operation) => {
        const rootScope = typeof window !== 'undefined' ? window : globalThis;
  ...
  const entryRetryTuning = runtimeOptions?.entryRetryTuning && typeof runtimeOptions.entryRetryTuning === 'object'
    ? runtimeOptions.entryRetryTuning
    : null;
...
async function waitForSignupPhoneEntryState(options = {}) {
  ...
  const switchToPhoneCooldownMs = normalizeTuningNumber(
    tuning.switchToPhoneCooldownMs ?? tuning.switchCooldownMs,
    1200,
    200,
    5000
  );
  ...
  while (Date.now() - start < timeout) {
    ...
    if (snapshot.state === 'email_entry') {
      ...
      if (switchToPhone && Date.now() - lastSwitchToPhoneAt >= switchToPhoneCooldownMs) {
        ...
        if (moreOptionsTrigger && Date.now() - lastMoreOptionsClickAt >= switchToPhoneCooldownMs) {
        ...
        } else if (!switchToPhone && !slowSnapshotLogged && Date.now() - start >= stateProbeThresholdMs) {
          slowSnapshotLogged = true;
    ...
    if (snapshot.state === 'entry_home' && snapshot.signupTrigger) {
      if (Date.now() - lastTriggerClickAt >= entryClickCooldownMs) {
        ...
        const entryClickDelaySeconds = (entryClickDelayMs / 1000).toFixed(entryClickDelayMs % 1000 === 0 ? 0 : 1);
        log(retryAttempt > 0
          ? `步骤 ${step}：上次点击后仍未进入手机号输入页，等待 ${entryClickDelaySeconds} 秒后重试点击官网注册入口（重试 ${retryAttempt}/${maxSignupEntryClickRetries}）...`
          : `步骤 ${step}：已找到官网注册入口，等待 ${entryClickDelaySeconds} 秒后点击...`);
        await sleep(entryClickDelayMs);
        throwIfStopped();
        const clickTarget = findSignupEntryTrigger({ allowHiddenFallback: false }) || snapshot.signupTrigger;
    ...
    if (!slowSnapshotLogged && Date.now() - start >= stateProbeThresholdMs) {
      slowSnapshotLogged = true;
      log(`步骤 ${step}：等待手机号注册入口超过 ${Math.floor(stateProbeThresholdMs / 1000)} 秒，页面诊断快照：${JSON.stringify(getSignupEntryDiagnostics())}`, 'warn');
    }
...
async function submitSignupPhoneNumberAndContinue(payload = {}) {
  ...
  const entryRetryTuning = payload?.step2EntryRetryTuning && typeof payload.step2EntryRetryTuning === 'object'
    ? payload.step2EntryRetryTuning
    : null;
  if (!phoneNumber) {
    throw new Error('未提供手机号，步骤 2 无法继续。');
  }

  const snapshot = await waitForSignupPhoneEntryState({ timeout: 25000, step: 2, entryRetryTuning });
  if (snapshot.state === 'password_page') {
...
async function step2_clickRegister(payload = {}) {
  // step2EntryRetryTuning 字段用于微调 Step2 的重试退避、最大重试次数、状态探测阈值。
  const entryRetryTuning = payload?.step2EntryRetryTuning && typeof payload.step2EntryRetryTuning === 'object'
    ? payload.step2EntryRetryTuning
    : null;
  ...
  const { email } = payload;
  return fillSignupEmailAndContinue(email, 2, { entryRetryTuning });
}
...
// 步骤 5 提交成功放行页（与 master 一致并补充 passkey enroll）。
function isStep5CompletionChatgptUrl(rawUrl = location.href) {
  ...
  try {
    ...
    if (['auth.openai.com', 'auth0.openai.com', 'accounts.openai.com'].includes(host)) {
      return /\/create-account-enroll-passkey(?:[/?#]|$)/i.test(path);
    }
    return false;
  } catch {
    return false;
  ...
  while (Date.now() - start < timeout && recoveryRound < maxRecoveryRounds) {
    ...
    if (snapshot.state === 'password') {
      ...
      if (snapshot.submitButton && isPasswordSubmitButtonReadyForRetry(snapshot.submitButton)) {
        ...
        const retrySettleStart = Date.now();
        while (Date.now() - retrySettleStart < 1200) {
          throwIfStopped();
          const postRetrySnapshot = inspectSignupVerificationState();
          if (postRetrySnapshot.state !== 'password') {
            break;
          }
          if (postRetrySnapshot.passwordErrorText) {
            break;
          }
          await sleep(150);
        }
...
async function fillVerificationCode(step, payload) {
  ...
  if (splitInputs?.length >= 6) {
    ...
    const splitSubmitBtn = await waitForVerificationSubmitButton(splitInputs[0], 2800).catch(() => null);
    if (splitSubmitBtn) {
    ...
    if (outcome.invalidCode) {
      ...
      // 进入手机号页时由后台第 9 步处理，这里不再额外输出提示日志。
    } else {
      if (typeof clearStep405RecoveryCount === 'function') clearStep405RecoveryCount(step);
  ...
  // Submit
  const submitBtn = await waitForVerificationSubmitButton(codeInput, 5800).catch(() => null);
...
async function step6SwitchToOneTimeCodeLogin(payload, snapshot) {
  ...
  const result = await waitForStep6SwitchTransition(loginVerificationRequestedAt, 11200, { visibleStep });
...
async function switchFromEmailPageToPhoneLogin(payload, snapshot) {
  ...
  if (!phoneEntryTrigger || !isActionEnabled(phoneEntryTrigger)) {
    ...
    if (moreOptionsTrigger && isActionEnabled(moreOptionsTrigger)) {
      ...
      const optionsSettleStart = Date.now();
      while (Date.now() - optionsSettleStart < 800) {
        throwIfStopped();
...
async function step6LoginFromPasswordPage(payload, snapshot) {
  ...
  if (currentSnapshot.passwordInput) {
    ...
    const passwordSettleStart = Date.now();
    while (Date.now() - passwordSettleStart < 500) {
      throwIfStopped();
      const latestSnapshot = normalizeStep6Snapshot(inspectLoginAuthState());
      if (latestSnapshot.state !== 'password_page') {
        break;
      }
      const latestPasswordInput = latestSnapshot.passwordInput || currentSnapshot.passwordInput;
      const expectedPassword = String(payload?.password || '');
      const passwordApplied = latestPasswordInput
        ? String(latestPasswordInput.value || '') === expectedPassword
        : true;
      const latestSubmitButton = latestSnapshot.submitButton || currentSnapshot.submitButton;
      if (passwordApplied && (!latestSubmitButton || isActionEnabled(latestSubmitButton))) {
        break;
      }
      await sleep(120);
    }
...
async function step6LoginFromEmailPage(payload, snapshot) {
  ...
  const emailSettleStart = Date.now();
  while (Date.now() - emailSettleStart < 500) {
    throwIfStopped();
    const latestSnapshot = normalizeStep6Snapshot(inspectLoginAuthState());
    if (latestSnapshot.state !== 'email_page') {
      break;
    }
    const latestEmailInput = latestSnapshot.emailInput || emailInput;
    const latestSubmitButton = latestSnapshot.submitButton || currentSnapshot.submitButton;
    const emailApplied = String(latestEmailInput?.value || '').trim() === String(payload?.email || '').trim();
    if (emailApplied && (!latestSubmitButton || isActionEnabled(latestSubmitButton))) {
      break;
    }
    await sleep(120);
  }
...
async function submitAddEmailAndContinue(payload = {}) {
  ...
  const addEmailSettleStart = Date.now();
  while (Date.now() - addEmailSettleStart < 500) {
    throwIfStopped();
    const liveEmailInput = getLoginEmailInput() || emailInput;
    const liveSubmitButton = getLoginSubmitButton({ allowDisabled: true }) || snapshot.submitButton;
    const emailApplied = String(liveEmailInput?.value || '').trim() === String(email || '').trim();
    if (emailApplied && (!liveSubmitButton || isActionEnabled(liveSubmitButton))) {
      break;
    }
    await sleep(120);
  }
...
async function waitForStep5SubmitOutcome(options = {}) {
  const {
    ...
    minRetryClickIntervalMs = 1200,
    adaptiveRetryWindowMs = 15000,
    fastPollIntervalMs = 120,
    steadyPollIntervalMs = 250,
    slowPollIntervalMs = 400,
  } = options;
  const start = Date.now();
  ...
  let lastStep5Error = '';
  const normalizedMaxRetryIntervalMs = Math.max(800, Math.floor(Number(retryClickIntervalMs) || 3500));
  const normalizedMinRetryIntervalMs = Math.max(
    600,
    Math.min(normalizedMaxRetryIntervalMs, Math.floor(Number(minRetryClickIntervalMs) || 1200))
  );
  const normalizedAdaptiveRetryWindowMs = Math.max(5000, Math.floor(Number(adaptiveRetryWindowMs) || 15000));
  const normalizedFastPollIntervalMs = Math.max(80, Math.floor(Number(fastPollIntervalMs) || 120));
  const normalizedSteadyPollIntervalMs = Math.max(
    normalizedFastPollIntervalMs,
    Math.floor(Number(steadyPollIntervalMs) || 250)
  );
  const normalizedSlowPollIntervalMs = Math.max(
    normalizedSteadyPollIntervalMs,
    Math.floor(Number(slowPollIntervalMs) || 400)
  );

  function getAdaptiveRetryIntervalMs() {
    const elapsedMs = Date.now() - start;
    if (elapsedMs <= normalizedAdaptiveRetryWindowMs && submitClickCount <= 1) {
      return normalizedMinRetryIntervalMs;
    }
    if (elapsedMs <= normalizedAdaptiveRetryWindowMs * 2 && submitClickCount <= 2) {
      return Math.min(
        normalizedMaxRetryIntervalMs,
        Math.round((normalizedMinRetryIntervalMs + normalizedMaxRetryIntervalMs) / 2)
      );
    }
    return normalizedMaxRetryIntervalMs;
  }

  function getAdaptivePollIntervalMs() {
    const elapsedMs = Date.now() - start;
    if (elapsedMs <= normalizedAdaptiveRetryWindowMs) {
      return normalizedFastPollIntervalMs;
    }
    if (elapsedMs <= normalizedAdaptiveRetryWindowMs * 2) {
      return normalizedSteadyPollIntervalMs;
    }
    return normalizedSlowPollIntervalMs;
  }
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    if (
      ...
      && Date.now() - lastSubmitClickAt >= getAdaptiveRetryIntervalMs()
    ) {
      ...
      if (isStep5SubmitButtonClickable(submitButton)) {
        ...
        lastSubmitClickAt = Date.now();
        const settleMs = Math.max(600, Math.min(1500, Math.round(getAdaptiveRetryIntervalMs() / 2)));
        await sleep(settleMs);
        continue;
      }
    }

    await sleep(getAdaptivePollIntervalMs());
  }
...
async function step5_fillNameBirthday(payload) {
  ...
  // Click "完成帐户创建" button
  const completeBtn = await waitForStep5SubmitButton(5500)
