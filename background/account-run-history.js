  ...
  function createAccountRunHistoryHelpers(deps = {}) {
    ...
    function normalizeFinalStatus(status = '') {
      ...
      if (
        ...
        || /_phone_verification_required$/.test(normalized)
        || /^node:[^:]+:phone_verification_required$/.test(normalized)
      ) {
        // 兼容历史写入的独立状态，回收为失败态展示与统计。
        return 'failed';
      }
      return '';
    }
    ...
    function normalizeAccountRunHistoryRecord(record) {
      ...
      const normalizedFinalStatus = normalizeFinalStatus(record.finalStatus || record.status || '');

      if (!accountIdentifier || !normalizedFinalStatus) {
        return null;
      }

      const rawFailureDetail = String(record.failureDetail || record.reason || '').trim();
      const rawFailureLabel = String(record.failureLabel || '').trim();
      const finalStatus = normalizedFinalStatus;
      const finishedAt = String(record.finishedAt || record.recordedAt || '').trim();
      const failureDetail = isFailureLikeStatus(finalStatus)
        ? rawFailureDetail
        : '';
      const failedStep = isFailureLikeStatus(finalStatus)
        ? resolveNormalizedFailedStep(record, failureDetail)
        : null;
      const failedNodeId = isFailureLikeStatus(finalStatus)
    ...
    function buildAccountRunHistoryRecord(state = {}, status = '', reason = '') {
      ...
      const normalizedFinalStatus = normalizeFinalStatus(status);
      ...
      const finalStatus = normalizedFinalStatus;
      const failureDetail = isFailureLikeStatus(finalStatus) ? rawReason : '';
      const failedStep = isFailureLikeStatus(finalStatus)
        ? extractRecordStep(status, failureDetail)
        : null;
      const statusNodeId = isFailureLikeStatus(finalStatus)
        ? extractRecordNodeFromStatus(status)
        : '';
      const stateNodeId = isFailureLikeStatus(finalStatus)
    ...
    async function clearAccountRunHistory(stateOverride = null, options = {}) {
      const state = stateOverride || await getState();
