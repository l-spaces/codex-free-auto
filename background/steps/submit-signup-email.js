(function attachBackgroundStep2(root, factory) {
  ...
  const STEP2_TAB_STABLE_MS_DEFAULT = 3000;
  const STEP2_TAB_STABLE_MS_FAST = 1200;

  function formatStableSeconds(ms) {
    const seconds = Math.max(0, Number(ms) || 0) / 1000;
    if (Math.abs(seconds - Math.round(seconds)) < 0.01) {
  ...
  function createStep2Executor(deps = {}) {
    const {
      ...
      step2TabStableMsDefault = STEP2_TAB_STABLE_MS_DEFAULT,
      step2TabStableMsFast = STEP2_TAB_STABLE_MS_FAST,
    ...
    async function waitForStep2SignupTabToSettle(tabId, logMessage) {
      ...
      const fallbackStableMs = Math.max(0, Math.floor(Number(step2TabStableMsDefault) || 0));
      let stableMs = fallbackStableMs;
      const fastStableMs = Math.max(0, Math.floor(Number(step2TabStableMsFast) || 0));
      if (fastStableMs > 0 && fastStableMs < fallbackStableMs) {
        const readyState = await getSignupEntryReadyState(tabId);
        if (isReadySignupEntryState(readyState)) {
          stableMs = fastStableMs;
        }
      }

      const stableSecondsText = formatStableSeconds(stableMs);
      await addLog(
        logMessage || `步骤 2：注册页标签已切换，正在等待页面加载完成并额外稳定 ${stableSecondsText} 秒...`,
        'info',
    ...
    async function ensureSignupTabForStep2() {
      ...
      if (!signupTabId || !(await isTabAlive('signup-page'))) {
        ...
        await waitForStep2SignupTabToSettle(
          signupTabId,
          '步骤 2：已切换到注册页标签，正在等待页面加载完成并确认页面稳定...'
        );
