(function attachBackgroundVerificationFlow(root, factory) {
  ...
  function createVerificationFlowHelpers(deps = {}) {
    ...
    function shouldEmitSampledLog(sampleKey, minIntervalMs) {
      ...
      if (sampledLogTimestamps.size > 300) {
    ...
    async function addSampledLog(message, level = 'info', options = {}, sampleKey = '', minIntervalMs = 0) {
      ...
      const key = String(sampleKey || `${level}|${normalizeVerificationLogMessage(message)}`);
    ...
    function getVerificationPollPayload(step, state, overrides = {}) {
      ...
      const isGptmailProvider = state?.mailProvider === GPTMAIL_PROVIDER;
      const gptmailTargetEmail = String(state?.currentGptmailAddress || state?.email || '').trim();
      const step8TargetEmail = String(state?.step8VerificationTargetEmail || '').trim()
        || (isGptmailProvider ? gptmailTargetEmail : state.email);
        ...
        while (true) {
          ...
          try {
            ...
            if (isTransportError) {
              ...
              await addSampledLog(
                ...
                'warn',
                {},
                `mail-transport-error:${step}:${mail?.source || mail?.provider || 'unknown'}`,
                3000
              );
            ...
            await addSampledLog(
              ...
              'warn',
              {},
              `mail-poll-error:${step}:${mail?.source || mail?.provider || 'unknown'}`,
              3000
            );
          }
          ...
          if (effectiveCooldownMs > 0) {
            await addSampledLog(
              ...
              'info',
              {},
              `mail-resend-cooldown:${step}:${round}`,
              5000
            );
    ...
    async function pollLuckmailVerificationCodeWithResend(step, state, pollOverrides = {}) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          ...
          await addSampledLog(
            `步骤 ${step}：LuckMail 暂未获取到新的${getVerificationCodeLabel(step)}验证码，等待 ${Math.ceil(intervalMs / 1000)} 秒后继续轮询 /code 接口（${attempt + 1}/${maxAttempts}）...`,
            'warn',
            {},
            `luckmail-code-poll-wait:${step}`,
            30000
          );
    ...
    async function pollFreshVerificationCode(step, state, mail, pollOverrides = {}) {
      ...
      if (mail.provider === GPTMAIL_PROVIDER) {
        const timedPoll = await applyMailPollingTimeBudget(step, {
          ...getVerificationPollPayload(step, state),
          ...cleanPollOverrides,
        }, cleanPollOverrides, `轮询${getVerificationCodeLabel(step)}验证码邮箱`);
        return pollGptmailVerificationCode(step, state, timedPoll.payload);
      }
      ...
      let lastError = null;
      const defaultVerificationPollPayload = getVerificationPollPayload(step, state);
      const maxAttemptsPerRound = Math.max(1, Math.floor(Number(defaultVerificationPollPayload.maxAttempts) || 1));
      let filterAfterTimestamp = cleanPollOverrides.filterAfterTimestamp ?? defaultVerificationPollPayload.filterAfterTimestamp;
      const maxResendRequests = resolveMaxResendRequests(pollOverrides);
      const maxRounds = maxResendRequests + 1;
      const initialPollMaxAttempts = Math.min(
        maxAttemptsPerRound,
        Math.max(0, Math.floor(Number(pollOverrides.initialPollMaxAttempts) || 0))
      );
      const configuredPollAttemptPlan = Array.isArray(pollOverrides.pollAttemptPlan)
        ? pollOverrides.pollAttemptPlan
          .map((value) => Math.min(maxAttemptsPerRound, Math.floor(Number(value) || 0)))
      ...
      if (mail.provider === HOTMAIL_PROVIDER) {
        ...
        for (let attempt = 1; attempt <= maxSubmitAttempts; attempt++) {
          ...
          await completeNodeFromBackground(completionNodeId, {
            ...
            code: result.code,
            // Step8 仅透传手机号需求标记，实际手机号页处理交给 Step9。
