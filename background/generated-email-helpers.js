(function attachGeneratedEmailHelpers(root, factory) {
  ...
  function createGeneratedEmailHelpers(deps = {}) {
    ...
    const DUCK_EMAIL_API_URL = 'https://quack.duckduckgo.com/api/email/addresses';
    const DUCK_EMAIL_API_TIMEOUT_MS = 15000;
    const DUCK_EMAIL_API_MAX_RETRIES = 2;
    ...
    function normalizeDuckEmailFromResponse(payload) {
      // 文档定义 address 可能是不带域名的本地部分，也可能是完整邮箱。
      ...
      if (candidate.includes('@')) {
        return /^[a-z0-9._%+-]+@duck\.com$/i.test(candidate) ? candidate : '';
      }

      if (!/^[a-z0-9._%+-]+$/i.test(candidate)) {
        return '';
      }
      return `${candidate}@duck.com`;
    }

    function buildDuckAuthorizationHeader(value = '') {
    ...
    async function requestDuckEmailFromApi(state = null, options = {}) {
      // DuckDuckGo 随机邮箱接口需要 Authorization，并固定 Origin / Referer。
      const authorizationHeader = buildDuckAuthorizationHeader(
        options.duckApiAuthorization ?? state?.duckApiAuthorization
      );
      if (!authorizationHeader) {
        throw new Error('Duck API Authorization 未配置，请先在侧边栏填写。');
      }

      const timeoutMs = Math.max(1000, Number(options.timeoutMs) || DUCK_EMAIL_API_TIMEOUT_MS);
      const maxRetries = Math.max(0, Math.floor(Number(options.maxRetries) || DUCK_EMAIL_API_MAX_RETRIES));
      ...
      for (let attempt = 1; attempt <= totalAttempts; attempt += 1) {
        ...
        let response;
        try {
          response = await fetch(DUCK_EMAIL_API_URL, {
            method: 'POST',
            headers: {
          ...
          const message = isTimeout
            ? `Duck API 请求超时（>${Math.round(timeoutMs / 1000)} 秒）`
            : `Duck API 请求失败：${err.message}`;
          if (attempt < totalAttempts) {
            await addLog(`Duck API：第 ${attempt} 次请求失败，正在重试...`, 'warn');
            continue;
          }
        ...
        if (!response.ok) {
          const payloadMessage = typeof payload === 'object' && payload
            ? (payload.message || payload.error || payload.msg)
            : '';
          const errorMessage = `Duck API 请求失败：${payloadMessage || `HTTP ${response.status}`}`;
          const retryable = response.status >= 500 || response.status === 429;
          if (attempt < totalAttempts && retryable) {
            await addLog(`Duck API：第 ${attempt} 次请求返回 ${response.status}，正在重试...`, 'warn');
            continue;
          }
          throw new Error(errorMessage);
        }

        const email = normalizeDuckEmailFromResponse(payload);
        if (!email) {
          throw new Error('Duck API 返回数据缺少 address 字段或格式无效。');
        }
        return email;
      }

      throw new Error('Duck API 请求失败：超过最大重试次数。');
    }

    async function fetchDuckEmail(options = {}) {
      throwIfStopped();
      const { state = null } = options;
      await addLog('Duck 邮箱：正在通过 API 生成新地址...');
      const email = await requestDuckEmailFromApi(state, options);

      await persistResolvedEmailState(state, email, {
        source: 'generated:duck',
        preserveAccountIdentity: Boolean(options?.preserveAccountIdentity),
      });
      await addLog(`Duck 邮箱：已生成 ${email}`, 'ok');
      return email;
    }
