...
- `手机号验证` 节点只属于邮箱注册模式的 OAuth 后置补手机号链路：
  - 已经在 OAuth 授权页则跳过。
  - 进入添加手机号页但当前流程未启用手机号验证节点则报错。
  - 启用手机号验证节点时才允许添加手机号并获取短信验证码。
...
- 手机号验证相关配置必须放在对应配置区；注册方式相关配置必须靠近 `注册方式`。
- 配置项显隐必须和真实可用模式一致：不可用时隐藏或禁用，不能只靠用户“不要点”。
- 持久配置、运行态输入、当前订单状态必须区分清楚；不能把验证流程手机号、账号身份手机号、后置验证手机号混成一个字段。
...
11. 如果改动影响 OAuth / 注册身份链路，我有没有逐项区分邮箱注册模式、手机号注册模式、OAuth 授权页、添加邮箱页、添加手机号页、邮箱验证码页、短信验证码页？
