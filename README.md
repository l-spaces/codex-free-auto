...
`FlowPilot` 是一个 Chrome 侧边栏扩展，用来批量处理 ChatGPT / OpenAI 账号注册、授权和平台接入流程。

它的定位不是“单个按钮脚本”，而是把注册、验证码、OAuth、账号绑定、自动重试和记录管理放进同一套可持续使用的工具里。
...
- 支持 `CPA`、`SUB2API`、`Codex2API` 三种 OpenAI 来源。
...
- 支持接码平台、手机号验证、自动重试、执行范围限制、贡献模式和账号记录面板。
