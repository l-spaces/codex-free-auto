...
- 侧栏提供一个独立运行态“注册手机号”输入框；第 2 步自动拿到号码后立即回填，用户手动接管手机号注册时也可以直接改写这一个运行态槽位。这个输入框表达账号身份，不等同于验证码订单；后续自动拉短信仍依赖 `signupPhoneActivation / signupPhoneCompletedActivation`
...
- 本轮联动自检重点：
  ...
  4. 第 4 步继续依赖当前手机号注册 activation 获取验证码；只有手机号字符串、没有 activation 时，只能作为页面填写来源，不能伪造成自动订单。
...
这条配置链路独立于 `mailProvider` 和 Hotmail 的服务模式。
...
1. 解析本轮应使用的注册方式：默认邮箱注册，普通模式下可切到手机号注册
2. 打开或复用注册页
...
7. 手机号注册分支：先点击官网“免费注册”并切换到手机号注册入口，必要时展开更多选项；确认手机号输入页就绪后，按“已有 signup activation > 手动填写的运行态手机号”的顺序决定手机号来源。内容脚本会通过 `content/phone-country-utils.js` 共享工具按号码最长区号和平台国家名切换手机号国家下拉框，兼容 `+(44)` / `(+44)` / `+44` 等页面文案；必须确认可视下拉按钮的区号已同步后才填写并提交，若仍显示旧国家则停止提交
...
3. OpenAI 手机号验证流程会按当前 sidepanel 中保存的配置进行申请或复用号码
4. 后台提交号码后，会按 `currentPhoneActivation.provider` 分发后续查码、完成、取消、ban、reuse
